...
<div class="bt-comment-wrapper">
	...
	<?php if (have_comments()) : ?>
		<h6 class="bt-heading-comment"><?php printf(
											esc_html__('%02d Comments', 'freska'),
											get_comments_number()
										);  ?></h6>
		...
		?>
			<nav class="comment-navigation" role="navigation">
				<h1 class="screen-reader-text"><?php esc_html_e('Comment navigation', 'freska'); ?></h1>
				<div class="nav-previous"><?php previous_comments_link(esc_html__('&larr; Older Comments', 'freska')); ?></div>
				<div class="nav-next"><?php next_comments_link(esc_html__('Newer Comments &rarr;', 'freska')); ?></div>
		...
		wp_list_comments(array(
			...
			'short_ping' => true,
			'avatar_size' => 150,
			'callback' => 'freska_custom_comment',
		));
		?>
	...
	?>
		<p class="no-comments"><?php esc_html_e('Comments are closed.', 'freska'); ?></p>
	<?php endif; ?>
	...
	$fields =  array(
		...
		'url' => '<div class="bt-form-url">
									...
									<input class="bt-form-url__input" name="url" type="text" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" placeholder="' . __('Website', 'freska') . '" />
								</div>',
	...
	$args = array(
		...
		'must_log_in' => '<div class="bt-must-log-in">' . esc_html__('You must be', 'freska') . ' <a href="' . wp_login_url(apply_filters('the_permalink', get_permalink())) . '">' . esc_html__('logged in', 'freska') . '</a> ' . esc_html__('to post a comment.', 'freska') . '</div>',

		'logged_in_as' => '<div class="bt-logged-in-as">' . esc_html__('Logged in as', 'freska') . ' <a class="bt-name" href="' . admin_url('profile.php') . '">' . $user_identity . '</a>. <a href="' . wp_logout_url(apply_filters('the_permalink', get_permalink())) . '" title="' . esc_attr__('Log out of this account', 'freska') . '">' . esc_html__('Log out?', 'freska') . '</a></div>',
