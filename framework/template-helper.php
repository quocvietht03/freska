...
if (!function_exists('freska_logo')) {
	function freska_logo($url = '', $height = 30)
	{
		if (!$url) {
			$url = get_template_directory_uri() . '/assets/images/site-logo.png';
		}
		echo '<a href="' . esc_url(home_url('/')) . '"><img class="logo" style="height: ' . esc_attr($height) . 'px; width: auto;" src="' . esc_url($url) . '" alt="' . esc_attr__('Logo', 'freska') . '"/></a>';
	}
}
	...
	?>
		<nav class="bt-pagination" role="navigation">
			...
			echo paginate_links(array(
				...
				'next_text' => esc_html__('Next', 'freska') . '<svg xmlns="http://www.w3.org/2000/svg" width="8" height="13" viewBox="0 0 8 13" fill="currentColor">
								<path d="M0.839282 12.4903C0.630446 12.2842 0.611461 11.9616 0.782327 11.7343L0.839282 11.6692L5.91327 6.6604L0.839282 1.65162C0.630446 1.44548 0.611461 1.1229 0.782327 0.895592L0.839282 0.830468C1.04812 0.624326 1.37491 0.605586 1.6052 0.774247L1.67117 0.830468L7.16137 6.24982C7.3702 6.45596 7.38919 6.77854 7.21832 7.00585L7.16137 7.07098L1.67117 12.4903C1.44145 12.7171 1.069 12.7171 0.839282 12.4903Z"></path>
								</svg>',
			));
			?>
 ...
 * Adds a back to top button to the footer
 */
if (!function_exists('freska_popup_newslleter_form')) {
	function freska_popup_newslleter_form()
	{
		if (function_exists('get_field')) {
			...
			$heading = !empty($newsletter_popup_setting['heading']) ? $newsletter_popup_setting['heading'] : esc_html__('Subscribe to get 10% OFF', 'freska');
			$subheading = !empty($newsletter_popup_setting['sub_heading']) ? $newsletter_popup_setting['sub_heading'] : esc_html__('Subscribe to our newsletter and receive 10% off your first purchase', 'freska');
			...
			$heading = esc_html__('Subscribe to get 10% OFF', 'freska');
			$subheading = esc_html__('Subscribe to our newsletter and receive 10% off your first purchase', 'freska');
			$note = '';
			$image_newsletter = '';
				...
				<div class="bt-newsletter-popup-content">
					...
					<div class="bt-newsletter-popup-image">
						<?php if (!empty($image_newsletter)): ?>
							<img src="<?php echo esc_url($image_newsletter); ?>" alt="<?php echo esc_attr__('Newsletter', 'freska'); ?>">
						<?php endif; ?>
					</div>
	...
	add_action('wp_footer', 'freska_popup_newslleter_form');
}
...
add_action( 'elementor/element/container/section_layout/after_section_end', function ( $element ) {
    ...
    $element->start_controls_section(
        'freska_container_blur',
        [
            'label' => __( 'Backdrop Blur', 'freska' ),
    ...
    // Enable Blur
    $element->add_control(
        'freska_backdrop_blur',
        [
            'label'        => __( 'Enable Blur', 'freska' ),
    ...
    // Blur Amount
    $element->add_control(
        'freska_backdrop_blur_amount',
        [
            'label' => __( 'Blur Amount', 'freska' ),
...
add_action('elementor/element/loop-carousel/section_carousel_pagination/before_section_end', function ($element) {
	$element->add_control(
		...
		[
			'type' => \Elementor\Controls_Manager::SWITCHER,
			'label' => esc_html__('Show Pagination Only Mobile?', 'freska'),
			'default' => 'no',
			'prefix_class' => 'bt-',
	...
	$element->add_control(
		...
		[
			'type' => \Elementor\Controls_Manager::SWITCHER,
			'label' => esc_html__('Use Theme Style?', 'freska'),
			'default' => 'no',
			'label_on' => esc_html__('Yes', 'freska'),
			'label_off' => esc_html__('No', 'freska'),
			'return_value' => 'yes',
	...
	$element->add_control(
		...
		[
			'label' => esc_html__('Position Horizontal', 'freska'),
			'type' => \Elementor\Controls_Manager::SELECT,
			'default' => 'left',
			'options' => [
				'left' => esc_html__('Left', 'freska'),
				'center' => esc_html__('Center', 'freska'),
				'right' => esc_html__('Right', 'freska'),
			],
	...
	$element->add_control(
		...
		[
			'label' => esc_html__('Position Vertical', 'freska'),
			'type' => \Elementor\Controls_Manager::SELECT,
			'default' => 'bottom',
			'options' => [
				'top' => esc_html__('Top', 'freska'),
				'bottom' => esc_html__('Bottom', 'freska'),
			],
			'condition' => [
				'pagination' => 'progressbar',
			],
	...
	$element->add_responsive_control(
		...
		[
			'label' => esc_html__('Top Position', 'freska'),
			'type' => \Elementor\Controls_Manager::SLIDER,
			'size_units' => ['px', '%', 'custom'],
			'range' => [
				'px' => [
	...
	$element->add_responsive_control(
		...
		[
			'label' => esc_html__('Bottom Position', 'freska'),
			'type' => \Elementor\Controls_Manager::SLIDER,
			'size_units' => ['px', '%', 'custom'],
			'range' => [
				'px' => [
...
add_action('elementor/element/loop-carousel/section_navigation_settings/before_section_end', function ($element) {
	$element->add_control(
		...
		[
			'type' => \Elementor\Controls_Manager::SWITCHER,
			'label' => esc_html__('Hidden Navigation Mobile?', 'freska'),
			'default' => 'no',
			'prefix_class' => 'bt-',
			'label_on' => esc_html__('Yes', 'freska'),
			'label_off' => esc_html__('No', 'freska'),
...
add_action('elementor/element/nested-tabs/section_title_style/before_section_end', function ($element) {
	$element->add_responsive_control(
		'heading_max_width',
		[
			'label' => esc_html__('Max Width', 'freska'),
			'type' => \Elementor\Controls_Manager::SLIDER,
			'size_units' => ['px', '%', 'custom'],
			'range' => [
				'px' => [
...
function freska_elwg_get_slider_settings($settings, $breakpoints)
{
...
function freska_elwg_get_grid_area_positions($collage_items, $breakpoints)
{
	$result = [
		'breakpoints' => []
	];
 ...
 * Preloader HTML
 */
if (!function_exists('freska_preloader_html')) {
	function freska_preloader_html()
	{
		if (!function_exists('get_field')) {
			return;
		}
 ...
 */
if (!function_exists('freska_add_preloader')) {
	function freska_add_preloader()
	{
