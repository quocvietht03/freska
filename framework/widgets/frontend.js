(function ($) {
	...
	**/
				...
				$.ajax({
					...
					success: function (response) {
						if (response.success) {
							...
							if (typeof window.FreskaCountdownProductSale === 'function') {
								window.FreskaCountdownProductSale($productsContainer);
							}
			...
			const performSearch = function () {
				...
				if (searchTerm.length >= 2) {
					...
					$.ajax({
						...
						url: AJ_Options.ajax_url,
						data: {
							action: 'freska_search_live_style1',
							search_term: searchTerm,
							category_slug: $catProductInput.val(),
							widget_category_include: categoryInclude,
							widget_category_exclude: categoryExclude,
						...
						success: function (response) {
							if (response.success) {
								setTimeout(function () {
									...
									// Initialize countdown timers for newly loaded products
									if (typeof window.FreskaCountdownProductSale === 'function') {
										window.FreskaCountdownProductSale($dataSearch);
									}
	...
	function FreskaAnimateText(selector, delayFactor = 50) {
		const $text = $(selector);
		...
		function checkIfElementInView() {
			...
			if (isElementInView) {
				if (!animationElement.hasClass('bt-animated')) {
					...
					FreskaAnimateText(animationElement, animationDelay);
				}
			}
		}
	...
	function FreskashowToast(idproduct, tools = 'cart', status = 'add') {
	...
	const SwitcherHandler = function ($scope, $) {
		const $switchers = $scope.find('.js-switcher-dropdown');
		if (!$switchers.length) {
			return;
		}
		...
		const isHrefActiveForCurrentUrl = (href) => {
			if (!href || href === '#') {
				return false;
			}
			...
			try {
				...
				const targetSearchEmpty = Array.from(targetUrlObj.searchParams.keys()).length === 0;
				if (targetUrlObj.pathname === '/' && targetSearchEmpty) {
					if (targetUrlObj.hash) {
						return currentUrlObj.hash === targetUrlObj.hash;
					}
					return false;
				}
		...
		const buildMergedUrl = (href) => {
			...
			// If target provides a hash, update it; otherwise keep current hash.
			if (targetUrl.hash) {
		...
		$switchers.each(function () {
			...
			if (!$currentItem.length || !$dropdownItems.length || !$dropdown.length) {
				return;
			}

			const switcherParamKeys = new Set();
			const switcherHashKeys = new Set();
			$dropdownItems.each(function () {
				...
				try {
					...
					urlObj.searchParams.forEach((value, key) => {
						switcherParamKeys.add(key);
					});
					// Track hash-only links (e.g. "#fr") so we can clear hash on reset.
					const targetSearchEmpty = Array.from(urlObj.searchParams.keys()).length === 0;
					if (urlObj.pathname === '/' && targetSearchEmpty && urlObj.hash) {
						switcherHashKeys.add(urlObj.hash);
					}
				} catch (e) { }
			});
			...
			$dropdownItems.each(function () {
				const href = $(this).attr('href');
				if (isHrefActiveForCurrentUrl(href)) {
					$matchedItem = $(this);
					return false;
				}
			});
			...
			if ($matchedItem && $matchedItem.length) {
				...
				$currentItem.html($matchedItem.html());
			}
			...
			$dropdownItems.on('click', function (e) {
				...
				if (href && href !== '#') {
					try {
						window.location.href = buildMergedUrl(href);
					} catch (err) {
					...
					try {
						...
						// If current URL hash belongs to this switcher, clear it on reset.
						if (urlObj.hash && switcherHashKeys.has(urlObj.hash)) {
							urlObj.hash = '';
							removedAny = true;
						}
	...
	function FreskaProductHotspotAddSetCart($container) {
		...
		if ($variationForm.length > 0) {
			$variationForm.find('.bt-attributes--item').each(function () {
				$(this).closest('.variations_form').off('show_variation.freskaloaditem').on('show_variation.freskaloaditem', function (event, variation) {
		...
		function updateHotspotProductPrice(productItem, variationId, $container) {
			...
			if ($addSetToCartBtn.length) {
				...
				// update total price
				totalPrice = Number(totalPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
		...
		$variationForm.find('select').on('change', function () {
			var $form = $(this).closest('.variations_form')
			$form.off('show_variation.freskachangeitem').on('show_variation.freskachangeitem', function (event, variation) {
				var variationId = variation.variation_id;
				if (variationId && variationId !== '0') {
		...
		$(document).on('freskaQuickViewLoaded', function () {
		...
		$container.find('.bt-button-add-set-to-cart').on('click', function (e) {
			...
			if (productIds.length > 0) {
				$.ajax({
					...
					url: AJ_Options.ajax_url,
					data: {
						action: 'freska_add_multiple_to_cart_variable',
	...
	const ProductTooltipHotspotHandler = function ($scope) {
		...
		if ($Hotspotslider.length > 0) {
			...
			const $hotspotrp = $HotspotProduct.closest('.bt-rp-section-hotspot');
		...
		FreskaProductHotspotAddSetCart($HotspotProduct);
	...
	const MegaMenuHandler = function ($scope, $) {
		...
		// Set --width-content-mega for vertical layout (desktop only)
		function setVerticalMenuContentWidth() {
			...
			var match = String(verticalMenuWidth).match(/^([\d.]+)(px)?$/);
			if (!match) return;

			var totalWidth = parseFloat(match[1]);
			var windowWidth = $(window).width();
			var edgePaddingPerSide = parseInt($megamenuWrapper.attr('data-vertical-menu-edge-padding'), 10) || 15;
			var edgePaddingTotal = edgePaddingPerSide * 2;
			var threshold = totalWidth + edgePaddingTotal;

			if (windowWidth <= threshold) {
				totalWidth = windowWidth - edgePaddingTotal;
			}
			var megamenuWidth = $megamenu.outerWidth();
			var $el = $megamenu;
			while (megamenuWidth === 0 && $el.length) {
				$el = $el.parent();
				megamenuWidth = $el.length ? $el.outerWidth() : 0;
			}
			var contentWidth = Math.max(0, totalWidth - megamenuWidth);

			$megamenu.css('--width-content-mega', contentWidth + 'px');
		}
	...
	function FreskaBundleCartAddSetCart($container) {
		const $variationForm = $container.find('.variations_form');
		const $productItems = $container.find('.bt-bundle-cart-product-list__item');
		if ($variationForm.length > 0) {
			$variationForm.find('.bt-attributes--item').each(function () {
				$(this).closest('.variations_form').off('show_variation.freskaloaditem').on('show_variation.freskaloaditem', function (event, variation) {
					var variationId = variation.variation_id;
					if (variationId && variationId !== '0') {
						var $ItemProduct = $(this).closest('.bt-bundle-cart-product-list__item');
						var $form = $(this).closest('.variations_form');
						var variations = $form.data('product_variations');
						if (variations) {
							var varObj = variations.find(function (v) {
								return v.variation_id === variationId;
							});

							if (varObj && varObj.price_html) {
								if ($ItemProduct.find(".bt-price").length > 0) {
									$ItemProduct.find(".bt-price").html(varObj.price_html);
								} else {
									$ItemProduct.find(".woocommerce-loop-product__infor .price").html(varObj.price_html);
								}
							}
						}
					}
		...
		function updateBundleCartProductPrice(productItem, variationId, $containerOuter) {
			const $productItem = productItem;
			const $productId = $productItem.data('product-id');
			const $product_currencySymbol = $productItem.data('product-currency');
			const $product_html_price_default = $productItem.attr('data-product-html-price-default');
			...
			const $vForm = $productItem.find('.variations_form');
			if (typeof variationId === 'undefined' || !variationId || typeof variationId === 'object') {
				variationId = parseInt($vForm.find('input.variation_id').val(), 10) || 0;
			}
			const $addSetToCartBtn = $containerOuter.find('.bt-button-add-set-to-cart');
			if ($addSetToCartBtn.length) {
				let idsData = $addSetToCartBtn.attr('data-ids');
				let idsArr = [];
				try {
				...
				idsArr.forEach(item => {
					...
					const $productItemId = $containerOuter.find(`.bt-bundle-cart-product-list__item[data-product-id="${item.product_id}"]`);
					let isInStock = true;
					const $form = $containerOuter.find(`.variations_form[data-product_id="${item.product_id}"]`);
					...
					if ($form.length) {
						if (!item.variation_id || item.variation_id === 0 || item.variation_id === null) {
							...
							if (variations) {
								const currentVariationId = parseInt(item.variation_id, 10);
								const matchedVar = variations.find(v => parseInt(v.variation_id, 10) === currentVariationId);
								if (matchedVar) {
									isInStock = matchedVar.is_in_stock ? true : false;
								} else {
									isInStock = false;
								}
							}
						...
						if ($form.length) {
							...
							if (variations) {
								...
								if (matchedVar && matchedVar.display_price) {
									totalPrice += parseFloat(matchedVar.display_price);
								}
							}
						} else {
							if ($productItemId.length) {
		...
		$productItems.each(function () {
			updateBundleCartProductPrice($(this), null, $container);
		});
		...
		$variationForm.find('select').on('change', function () {
			...
			$form.off('show_variation.freskachangeitem').on('show_variation.freskachangeitem', function (event, variation) {
				var variationId = variation.variation_id;
				if (variationId && variationId !== '0') {
					var $ItemProduct = $form.closest('.bt-bundle-cart-product-list__item');
					if (!variation.is_in_stock) {
						$ItemProduct.addClass('out-of-stock');
						$ItemProduct.attr('data-in-stock', '0');
					} else {
					...
					updateBundleCartProductPrice($ItemProduct, variationId, $container);
					var variations = $form.data('product_variations');
					if (variations) {
						var varObj = variations.find(function (v) {
							return v.variation_id === variationId;
		...
		$container.find('.bt-button-add-set-to-cart').off('click.bundlecart').on('click.bundlecart', function (e) {
			e.preventDefault();
			const $this = $(this);
			if ($this.hasClass('disabled')) {
				return;
			...
			let productIds = $this.data('ids');
			if (typeof productIds === 'string') {
				try { productIds = JSON.parse(productIds); } catch (err) { productIds = []; }
			}
			if (!Array.isArray(productIds)) { productIds = []; }

			productIds = productIds.filter(item => {
				const $productItem = $container.find(`.bt-bundle-cart-product-list__item[data-product-id="${item.product_id}"]`);
				if ($productItem.length) {
					if ($productItem.hasClass('out-of-stock')) { return false; }
					const inStockAttr = $productItem.data('in-stock');
					if (inStockAttr === 0 || inStockAttr === '0') { return false; }
				}
				return true;
			});
			...
			if (productIds.length > 0) {
				$.ajax({
					...
					success: function (response) {
						...
						if (response.success) {
							$(document.body).trigger('updated_wc_div');
							if(typeof FreskaFreeShippingMessage === 'function') FreskaFreeShippingMessage();
							$this.html('View Cart');
							$this.addClass('bt-view-cart');
							productIds.forEach((item, idx) => {
								const productId = item.variation_id && item.variation_id !== 0 ? item.variation_id : item.product_id;
								setTimeout(() => {
									if(typeof FreskaHandleCartAction === 'function') FreskaHandleCartAction(productId);
								}, idx * 300);
							});
						}
					}
		...
		$variationForm.each(function() {
			...
			selects.forEach(function(select) {
				if (select.dataset.swatchesReady) return;
				select.style.display = 'none';
				
				var wrapper = document.createElement('div');
				wrapper.className = 'bt-variation-swatches';
				
				Array.from(select.options).forEach(function(option) {
					if (!option.value) return; 
					
					var pill = document.createElement('div');
					pill.className = 'bt-swatch-pill';
					...
					pill.addEventListener('click', function(e) {
						e.preventDefault();
						wrapper.querySelectorAll('.bt-swatch-pill').forEach(function(el) { el.classList.remove('active'); });
						pill.classList.add('active');
				...
				select.dataset.swatchesReady = 'true';
			});
		});
		
		$variationForm.on('woocommerce_update_variation_values reset_data', function() {
			var forms = $(this);
			forms.each(function() {
				var selects = this.querySelectorAll('select');
				selects.forEach(function(select) {
					var wrapper = select.parentNode.querySelector('.bt-variation-swatches');
					if (wrapper) {
						wrapper.querySelectorAll('.bt-swatch-pill').forEach(function(pill) {
	...
	const BundleCartHandler = function ($scope) {
		const $bundleCart = $scope.find('.bt-elwg-bundle-cart--default');
		if ($bundleCart.length > 0) {
			FreskaBundleCartAddSetCart($bundleCart);
		}
	};
	...
	$(window).on('elementor/frontend/init', function () {
		...
		elementorFrontend.hooks.addAction('frontend/element_ready/bt-bundle-cart.default', BundleCartHandler);
		...
		elementorFrontend.hooks.addAction('frontend/element_ready/bt-locale-switcher.default', SwitcherHandler);
