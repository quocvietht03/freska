...
namespace FreskaElementorWidgets\Widgets\BannerProductSlider;
...
class Widget_BannerProductSlider extends Widget_Base
{
    ...
    {
        return __('Banner Product Slider', 'freska');
    }
    ...
    protected function register_content_section_controls()
    {
        ...
        $repeater->add_control(
            'enable_video_hover',
            [
                'label' => __('Enable Video on Hover', 'freska'),
        ...
        $repeater->add_control(
            'video_type',
            [
                'label' => __('Video Type', 'freska'),
                'type' => Controls_Manager::SELECT,
                'default' => 'url',
                'options' => [
                    'url' => __('URL (Mp4)', 'freska'),
                    'file' => __('Media File', 'freska'),
                ],
                'condition' => [
        ...
        $repeater->add_control(
            'video_url',
            [
                'label' => __('Video URL', 'freska'),
                'type' => Controls_Manager::TEXT,
                'placeholder' => __('Enter video URL', 'freska'),
                'description' => __('Enter video URL (YouTube or Vimeo)', 'freska'),
                'condition' => [
                    'video_type' => ['url'],
                    'enable_video_hover' => 'yes',
                ],
        ...
        $repeater->add_control(
            'video_file',
            [
                'label' => __('Choose Video File', 'freska'),
                'type' => Controls_Manager::MEDIA,
                'media_type' => 'video',
                'condition' => [
                    'video_type' => 'file',
        ...
        $this->start_controls_section(
            'section_slider',
            [
                'label' => esc_html__('Slider', 'freska'),
            ]
        );
                                    ...
                                    <div class="bt-cover-image">
                                        ...
                                        if (!empty($item['banner_image']['id'])) {
                                            ...
                                            if (!empty($item['banner_image']['url'])) {
                                                echo '<img src="' . esc_url($item['banner_image']['url']) . '" alt="' . esc_html__('Awaiting product image', 'freska') . '">';
                                            } else {
                                                echo '<img src="' . esc_url(Utils::get_placeholder_image_src()) . '" alt="' . esc_html__('Awaiting product image', 'freska') . '">';
                                            }
                                        } ?>
                                                ...
                                                <div class="bt-product-content">
                                                    ...
                                                    <div class="bt-product-add-to-cart">
                                                        <?php if ($product->is_type('simple') && $product->is_purchasable() && $product->is_in_stock()) : ?>
                                                            <a href="?add-to-cart=<?php echo esc_attr($product->get_id()); ?>" aria-describedby="woocommerce_loop_add_to_cart_link_describedby_<?php echo esc_attr($product->get_id()); ?>" data-quantity="1" class="bt-button product_type_simple add_to_cart_button ajax_add_to_cart bt-button-hover" data-product_id="<?php echo esc_attr($product->get_id()); ?>" data-product_sku="" rel="nofollow"><?php echo esc_html__('Add to cart', 'freska') ?></a>
                                                        <?php else : ?>
                                                            <a href="<?php echo esc_url($product->get_permalink()); ?>" class="bt-button bt-view-product"><?php echo esc_html__('View Product', 'freska'); ?></a>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                            </div>
