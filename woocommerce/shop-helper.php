...
add_action('freska_woocommerce_show_product_loop_sale_flash', 'woocommerce_show_product_loop_sale_flash', 10);
...
add_action('freska_woocommerce_template_single_price', 'woocommerce_template_single_price', 10);
add_action('freska_woocommerce_template_single_excerpt', 'woocommerce_template_single_excerpt', 20);
add_action('freska_woocommerce_template_single_add_to_cart', 'woocommerce_template_single_add_to_cart', 30);
add_action('woocommerce_before_add_to_cart_quantity', 'freska_size_guide_button_before_quantity', 5);
...
add_action('freska_woocommerce_template_single_sharing', 'woocommerce_template_single_sharing', 50);
add_action('freska_checkout_review', 'woocommerce_order_review', 10);
add_action('freska_checkout_order', 'woocommerce_checkout_payment', 20);
add_action('freska_woocommerce_template_cross_sell', 'woocommerce_cross_sell_display', 50);
...
if (!function_exists('freska_woocommerce_template_loop_product_title')) {
    function freska_woocommerce_template_loop_product_title($parent_id = null)
    {
        global $product;

        if ($parent_id === null && $product && $product->is_type('variation')) {
            $parent_id = $product->get_parent_id();
        }

        if (!empty($parent_id) && $parent_id > 0) {
            echo '<h2 class="' . esc_attr(apply_filters('woocommerce_product_loop_title_classes', 'woocommerce-loop-product__title')) . '">' . esc_html(get_the_title($parent_id)) . '</h2>';
        } else {
...
add_action('freska_woocommerce_template_loop_product_default_attributes', 'freska_woocommerce_template_loop_product_default_attributes', 10, 1);
if (!function_exists('freska_woocommerce_template_loop_product_default_attributes')) {
    /**
     * Display default attributes for variable products (passed from product tooltip hotspot widget)
     *
     * @param array|null $attributes_default Default attributes as attribute_name => value
     */
    function freska_woocommerce_template_loop_product_default_attributes($attributes_default = null)
    {
        global $product;
        ...
        if (!empty($attributes_default) && is_array($attributes_default)) {
            echo '<ul class="bt-product-default-attributes">';
            $attribute_pairs = array();
            foreach ($attributes_default as $attribute_name => $attribute_value) {
                $attribute_label = wc_attribute_label($attribute_name);
                $attribute_pairs[] = '<li><span class="attribute-name">' . esc_html($attribute_label) . '</span>: ' . esc_html($attribute_value) . '</li>';
            }
...
/**
 * Get variation attributes as formatted HTML list.
 *
 * @param int $variation_id Product variation ID.
 * @return string HTML string (ul/li) or empty string.
 */
if (!function_exists('freska_get_variation_attributes_html')) {
    function freska_get_variation_attributes_html($variation_id)
    {
        ...
        foreach ($attrs as $name => $value) {
            $attr_name = str_replace('attribute_', '', $name);
            ...
            if ('' !== $value) {
                $items[] = '<li><span class="attribute-name">' . esc_html(wc_attribute_label($attr_name, $variation_product)) . '</span>: ' . esc_html(rawurldecode($value)) . '</li>';
            }
        }
        ...
        return '<ul class="bt-product-variation">' . implode('', $items) . '</ul>';
    }
}

add_action('freska_woocommerce_template_loop_product_variation', 'freska_woocommerce_template_loop_product_variation', 10, 1);
if (!function_exists('freska_woocommerce_template_loop_product_variation')) {
    /**
     * Display formatted variation attributes for variation products (passed from cart/cross-sells.php, up-sells, etc.)
     *
     * @param int|null $parent_id Parent product ID when displaying variation in loop
     */
    function freska_woocommerce_template_loop_product_variation($parent_id = null)
    {
        global $product;

        if (!$product || !$product->is_type('variation') || empty($parent_id) || $parent_id <= 0) {
            return;
        }
        echo freska_get_variation_attributes_html($product->get_id());
    }
}

add_action('freska_woocommerce_template_loop_product_short_description', 'freska_woocommerce_template_loop_product_short_description', 10);
if (!function_exists('freska_woocommerce_template_loop_product_short_description')) {
    /**
     * Display product short description in loop (archive/shop/category or AJAX filter only)
     */
    function freska_woocommerce_template_loop_product_short_description()
    {
        ...
        $show_short_desc = (is_archive() && 'product' === get_post_type()) || !empty($is_ajax_filter_product);
        if (!$show_short_desc) {
            return;
        }
 ...
 */
add_filter('woocommerce_loop_add_to_cart_link', 'freska_woocommerce_loop_add_to_cart_link', 10, 3);
if (!function_exists('freska_woocommerce_loop_add_to_cart_link')) {
    function freska_woocommerce_loop_add_to_cart_link($link, $product, $args)
    {
...
add_filter('request', 'freska_remove_brand_from_query_vars', 1, 1);
 ...
 */
function freska_is_category_archive_page()
{
...
function freska_shop_categories_display()
{
    // Use the helper functions to determine what to show
    if (!freska_should_show_categories()) {
        return;
    }

    // Determine if we're on shop page or category page to get correct display mode
    $is_category_page = isset($_GET['product_cat']) || freska_is_category_archive_page();
    ...
    if ('subcategories' === $display_type || 'both' === $display_type) {
        ...
        woocommerce_output_product_categories(
            array(
                'parent_id' => freska_is_category_archive_page() ? get_queried_object_id() : 0,
            )
        );
...
add_action('freska_shop_categories_display', 'freska_shop_categories_display');

function freska_should_show_categories()
{
    ...
    if ($is_category_page) {
        ...
        } elseif (freska_is_category_archive_page()) {
            $current_category = get_queried_object();
        }
...
function freska_should_show_products()
{
    $shop_page_display = get_option('woocommerce_shop_page_display', '');
    if (isset($_GET['layout-shop']) && $_GET['layout-shop'] == 'show-categories') {
        $shop_page_display = 'subcategories';
 ...
 */
function freska_woocommerce_single_product_meta()
{
    global $product;
    ...
    $sku = $product->get_sku();
    if ($sku) {
        echo '<li class="sku"><span>' . esc_html__('SKU:', 'freska') . '</span> ' . esc_html($sku) . '</li>';
    }
    ...
    if ($author) {
        echo '<li class="vendor"><span>' . esc_html__('Vendor:', 'freska') . '</span> ' . esc_html($author) . '</li>';
    }

    $availability = $product->is_in_stock() ? esc_html__('In stock', 'freska') : esc_html__('Out of stock', 'freska');
    echo '<li class="availability"><span>' . esc_html__('Availability:', 'freska') . '</span> ' . esc_html($availability) . '</li>';
    ...
    if ($terms && !is_wp_error($terms)) {
        ...
        echo '<li class="categories"><span>' . esc_html__('Categories:', 'freska') . '</span> ' . implode(', ', $cat_links) . '</li>';
    }
    echo '</ul>';
}

// Size Guide Button - Display before quantity (for simple products or products without size)
function freska_size_guide_button_before_quantity()
{
...
function freska_size_guide_popup_content()
{
    if (!is_product()) {
        return;
    }
        ...
        <div id="bt-size-guide-popup" class="bt-size-guide-popup mfp-content__popup mfp-hide ">
            <div class="bt-size-guide-popup-content mfp-content__inner">
                <div class="bt-size-guide-popup-header">
                    <h3><?php echo esc_html__('Size Guide', 'freska'); ?></h3>
                </div>
...
add_action('wp_footer', 'freska_size_guide_popup_content');

// custom product loop image
add_action('freska_woocommerce_template_loop_product_thumbnail', 'freska_woocommerce_template_loop_product_thumbnail', 10);

function freska_woocommerce_template_loop_product_thumbnail()
{
    global $product;
    ...
    if ($post_thumbnail_id) {
        ...
        $html = freska_get_gallery_image_html($post_thumbnail_id, false, false);

        // If there are gallery images, show the first one
        $attachment_ids = $product->get_gallery_image_ids();

        if (!empty($attachment_ids) && isset($attachment_ids[0])) {
            $html .= freska_get_gallery_image_html($attachment_ids[0], false, false);
        } else {
        ...
        $html .= sprintf('<img src="%s" alt="%s" class="wp-post-image" />', esc_url(wc_placeholder_img_src('woocommerce_thumbnail')), esc_html__('Awaiting product image', 'freska'));
        $html .= '</div>';
...
add_action('woocommerce_cart_updated', 'freska_redirect_after_add_to_cart');

function freska_redirect_after_add_to_cart()
{
...
add_action('freska_template_loop_product_countdown_and_sale', 'freska_display_product_countdown_and_sale_marquee', 10);

function freska_display_product_countdown_and_sale_marquee()
{
    global $product;
        ...
        <div class="bt-product-countdown-timer bt-countdown-product-sale">
            <div class="bt-countdown bt-countdown-product-js"
                ...
                <div class="bt-countdown--item">
                    ...
                    <span class="bt-countdown--label"><?php esc_html_e('Days', 'freska'); ?></span>
                </div>
                <div class="bt-delimiter">:</div>
                <div class="bt-countdown--item">
                    <span class="bt-countdown--digits bt-countdown-hours">--</span>
                    <span class="bt-countdown--label"><?php esc_html_e('Hours', 'freska'); ?></span>
                </div>
                <div class="bt-delimiter">:</div>
                <div class="bt-countdown--item">
                    <span class="bt-countdown--digits bt-countdown-mins">--</span>
                    <span class="bt-countdown--label"><?php esc_html_e('Mins', 'freska'); ?></span>
                </div>
                <div class="bt-delimiter">:</div>
                <div class="bt-countdown--item">
                    <span class="bt-countdown--digits bt-countdown-secs">--</span>
                    <span class="bt-countdown--label"><?php esc_html_e('Secs', 'freska'); ?></span>
                </div>
            </div>
        </div>
        <?php
        ...
        if ($enable_sale_marquee === 'yes') {
            ...
            if ($percentage > 0) {
                $sale_text = sprintf(
                    ...
                    esc_html__('Hot Sale', 'freska'),
                    esc_html($percentage),
                    esc_html__('off', 'freska')
                );
...
add_filter('woocommerce_sale_flash', 'freska_woocommerce_percentage_sale', 10, 3);

function freska_woocommerce_percentage_sale($html, $post, $product)
{
    if ($product->is_type('variable')) {
        $percentages = array();
    ...
    if (is_product() && is_single()) {
        return '<span class="onsale">-' . $percentage . ' ' . esc_html__('off', 'freska') . '</span>';
    } else {
        return '<span class="onsale">-' . $percentage . '</span>';
    }
}

add_filter('woocommerce_pagination_args', 'freska_woocommerce_pagination_args');

function freska_woocommerce_pagination_args()
{
    ...
    return array(
        ...
        'next_text' => esc_html__('Next', 'freska') . '<svg width="19" height="16" viewBox="0 0 19 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
add_filter('woocommerce_output_related_products_args', 'freska_woocommerce_related_products_args', 20);

function freska_woocommerce_related_products_args($args)
{
...
function freska_remove_after_single_product_summary()
{
    if (function_exists('get_field')) {
        $related_posts = get_field('product_related_posts', 'options');
...
add_action('init', 'freska_remove_after_single_product_summary');

/* Sold Product */
function freska_woocommerce_item_sold($product_id)
{
    global $post;
    $args = array(
        'status' => 'completed',
  ...
  <path d="M17.106 9.80077L8.35603 19.1758C8.26331 19.2747 8.14091 19.3408 8.00731 19.3641C7.87372 19.3874 7.73617 19.3666 7.61543 19.3049C7.49468 19.2432 7.3973 19.1438 7.33796 19.0219C7.27863 18.8999 7.26057 18.762 7.2865 18.6289L8.43181 12.9L3.92947 11.2094C3.83277 11.1732 3.74655 11.1136 3.67849 11.036C3.61043 10.9584 3.56266 10.8651 3.53944 10.7645C3.51623 10.6639 3.5183 10.5591 3.54546 10.4595C3.57262 10.3599 3.62403 10.2686 3.69509 10.1937L12.4451 0.818744C12.5378 0.719788 12.6602 0.653675 12.7938 0.630383C12.9274 0.60709 13.065 0.627882 13.1857 0.68962C13.3064 0.751359 13.4038 0.850694 13.4632 0.972636C13.5225 1.09458 13.5406 1.23251 13.5146 1.36562L12.3662 7.10077L16.8685 8.78906C16.9645 8.82547 17.05 8.88496 17.1176 8.96228C17.1851 9.0396 17.2326 9.13236 17.2557 9.23237C17.2789 9.33237 17.2771 9.43655 17.2504 9.53569C17.2238 9.63482 17.1731 9.72587 17.1029 9.80077H17.106Z" fill="#C72929"/>
    ...
    if ($total_quantity_sold > 1) {
        echo esc_html__(' items sold', 'freska');
    } else {
        echo esc_html__(' item sold', 'freska');
    }
    echo '</div>';
}
...
function freska_add_review_title_meta_box()
{
    add_meta_box(
        ...
        __('Review Title', 'freska'),
        'freska_render_review_title_meta_box',
        'comment',
        'normal',
        'high'
    );
...
add_action('add_meta_boxes_comment', 'freska_add_review_title_meta_box');

// Render meta box content
function freska_render_review_title_meta_box($comment)
{
    $review_title = get_comment_meta($comment->comment_ID, 'review_title', true);
    wp_nonce_field('freska_review_title_update', 'review_title_nonce');
    ?>
    <p>
        <label for="review_title"><?php esc_html_e('Review Title', 'freska'); ?></label><br>
...
function freska_save_review_title($comment_id)
{
    // For admin form submission
    if (isset($_POST['review_title_nonce']) && wp_verify_nonce($_POST['review_title_nonce'], 'freska_review_title_update')) {
...
add_action('comment_post', 'freska_save_review_title');
add_action('edit_comment', 'freska_save_review_title');
...
function freska_product_create_pages_support()
{
...
add_action('init', 'freska_product_create_pages_support', 1);

function freska_get_products_by_rating($rating)
{
    $args = [
        'post_type' => 'product',
        'posts_per_page' => -1,
    ...
    if (freska_is_category_archive_page()) {
        $current_category = get_queried_object();
...
function freska_get_nested_category_children($parent_id, $taxonomy, $all_terms = null)
{
    if ($all_terms === null) {
    ...
    foreach ($all_terms as $term) {
        if (isset($term->parent) && intval($term->parent) === intval($parent_id)) {
            ...
            $grandchildren = freska_get_nested_category_children($term->term_id, $taxonomy, $all_terms);
...
function freska_render_nested_category($term, $slug, $field_value, $parent_url = '', $level = 0)
{
        ...
        <?php if ($has_children) { ?>
            ...
            <div class="bt-children-categories">
                <?php foreach ($term->children as $child) {
                    freska_render_nested_category($child, $slug, $field_value, $url_category, $level + 1);
                } ?>
            </div>
        <?php } ?>
    </div>
...
/* Field Product */
function freska_product_field_radio_html($slug = '', $field_title = '', $field_value = '')
{
    if (empty($slug)) {
        return;
    }
    ...
    if (!empty($terms) && !is_wp_error($terms)) {
        ...
        if ($slug === 'product_cat' && $category_mode === 'parent') {
            ...
            if (!is_wp_error($all_terms) && !empty($all_terms)) {
                ...
                foreach ($terms as $term) {
                    $term->children = freska_get_nested_category_children($term->term_id, $slug, $all_terms);
                }
            }
        }
    ?>
        <div class="bt-form-field bt-field-type-radio <?php echo 'bt-field-' . esc_attr($slug); ?> bt-field-mode-<?php echo esc_attr($category_mode); ?>" data-name="<?php echo esc_attr($slug); ?>">
            ...
            <div class="bt-field-list">
                <?php foreach ($terms as $term) {
                    freska_render_nested_category($term, $slug, $field_value, '', 0);
                } ?>
            </div>
        </div>
    <?php
 ...
 */
function freska_get_term_count_in_category($term, $taxonomy)
{
...
function freska_product_field_multiple_html($slug = '', $field_title = '', $field_value = '')
{
    if (empty($slug)) {
        return;
    }
    ...
    if (strpos($slug, 'pa_') === 0) {
        if (!function_exists('freska_is_attribute_enabled_in_search')) {
            return;
        }
        if (!freska_is_attribute_enabled_in_search($slug)) {
            return;
        }
    }
    ...
    if (!empty($terms) && !is_wp_error($terms)) {
        ...
        foreach ($terms as $term) {
            ...
            $term_count = freska_get_term_count_in_category($term, $slug);
            // Check if count > 0
            if ($term_count > 0) {
...
function freska_product_field_multiple_color_html($slug = '', $field_title = '', $field_value = '')
{
    if (empty($slug)) {
        return;
    }
        ...
        <div class="bt-form-field bt-field-type-multi bt-field-color" data-name="<?php echo esc_attr($slug); ?>">
            ...
            <div class="bt-field-list">
                ...
                foreach ($terms as $term) {
                    ...
                    $color = get_term_meta($term_id, 'freska_term_color', true);
                    if (!$color) {
                        $color = $term->slug;
                    }
                ?>
...
function freska_product_field_price_slider($field_title = '', $field_min_value = '', $field_max_value = '')
{
    $prices = freska_highest_and_lowest_product_price();
        ...
        <div class="bt-field-price-slider-wrapper">
            ...
            <div class="bt-field-price-options">
                <div class="bt-field-min-price">
                    <label for="bt-min-price"><?php esc_html_e('Min price', 'freska') ?></label>
                ...
                <div class="bt-field-max-price">
                    <label for="bt-max-price"><?php esc_html_e('Max price', 'freska') ?></label>
...
function freska_product_field_rating($slug = '', $field_title = '', $field_value = '')
{
    if (empty($slug)) {
        return;
    }
            ...
            ?>
                <?php if ($rating == $field_value) { ?>
                    <div class="item-rating">
                        ...
                        ?>
                        <span class="bt-count"><?php echo freska_get_products_by_rating($rating) ?></span>
                    </div>
                <?php } else { ?>
                    <div class="item-rating">
                        <span class="check-rating"></span>
...
function freska_highest_and_lowest_product_price()
{
...
function freska_product_pagination($current_page, $total_page)
{
    if (1 >= $total_page) {
        return;
    }
    ...
    <nav class="bt-pagination bt-product-pagination" role="navigation">
        <?php if (1 != $current_page) { ?>
            <a class="prev page-numbers" href="#" data-page="<?php echo esc_attr($current_page - 1); ?>"><svg xmlns="http://www.w3.org/2000/svg" width="8" height="13" viewBox="0 0 8 13" fill="currentColor">
                ...
                </svg> <?php echo esc_html__('Prev', 'freska'); ?></a>
        <?php } ?>
        ...
        <?php if ($total_page != $current_page) { ?>
            <a class="next page-numbers" href="#" data-page="<?php echo esc_attr($current_page + 1); ?>"><?php echo esc_html__('Next', 'freska'); ?><svg xmlns="http://www.w3.org/2000/svg" width="8" height="13" viewBox="0 0 8 13" fill="currentColor">
                    <path d="M0.839282 12.4903C0.630446 12.2842 0.611461 11.9616 0.782327 11.7343L0.839282 11.6692L5.91327 6.6604L0.839282 1.65162C0.630446 1.44548 0.611461 1.1229 0.782327 0.895592L0.839282 0.830468C1.04812 0.624326 1.37491 0.605586 1.6052 0.774247L1.67117 0.830468L7.16137 6.24982C7.3702 6.45596 7.38919 6.77854 7.21832 7.00585L7.16137 7.07098L1.67117 12.4903C1.44145 12.7171 1.069 12.7171 0.839282 12.4903Z"></path>
                </svg></a>
        <?php } ?>
    </nav>
...
function freska_product_load_more_button($current_page, $total_page)
{
...
?>
    <div class="bt-load-more-button-wrap">
        <button class="bt-load-more-btn" data-page="<?php echo esc_attr($current_page + 1); ?>" data-total="<?php echo esc_attr($total_page); ?>">
            <span class="bt-btn-text"><?php echo esc_html__('Load More Products', 'freska'); ?></span>
            <span class="bt-btn-loading" style="display: none;">
                ...
                <?php echo esc_html__('Loading...', 'freska'); ?>
            </span>
        </button>
    </div>
    <?php
...
function freska_products_query_args($params = array(), $limit = 9)
{
    $query_args = array(
        'post_type' => 'product',
        'post_status' => 'publish',
    ...
    $enabled_attributes = function_exists('freska_get_all_enabled_search_attributes') ? freska_get_all_enabled_search_attributes() : array();
    ...
    $color_taxonomies = function_exists('freska_get_all_color_taxonomies') ? freska_get_all_color_taxonomies() : array();
...
function freska_products_filter()
{
    ...
    $limit = $rows * $columns;
    $query_args = freska_products_query_args($_POST, $limit);
    ...
    ob_start();
    if ($total_products > 0) {
        $product_text = ($total_products == 1) ? __('%s Product Found', 'freska') : __('%s Products Found', 'freska');
        printf(
            $product_text,
            '<span class="highlight">' . esc_html($total_products) . '</span>'
        );
    } else {
        echo esc_html__('No results', 'freska');
    }
    $output['results'] = ob_get_clean();
    // update button Results
    $product_text = ($total_products == 1) ? __('Show %s Product', 'freska') : __('Show %s Products', 'freska');
    printf($total_products > 0 ? $product_text : esc_html__('No products found', 'freska'), $total_products);
    ...
    if ($wp_query->have_posts()) {
        ...
        if ($pagination_type === 'button-load-more') {
            $output['pagination'] = freska_product_load_more_button($current_page, $total_page);
            ...
            if ($current_page < $total_page) {
                $output['pagination'] = '<div class="bt-infinite-scroll-trigger" data-page="' . esc_attr($current_page + 1) . '" data-total="' . esc_attr($total_page) . '">'
                    . '<span class="bt-loading-spinner" style="display:none;">' . esc_html__('Loading more...', 'freska') . '</span>'
                    . '</div>';
            } else {
                $output['pagination'] = '';
            }
        } else {
            $output['pagination'] = freska_product_pagination($current_page, $total_page);
        }
        ...
        $output['items'] = '<div class="not-found-products"><svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" fill="currentColor" viewBox="0 0 256 256"><path d="M128,24A104,104,0,1,0,232,128,104.13,104.13,0,0,0,128,24Zm-18.34,98.34a8,8,0,0,1-11.32,11.32L88,123.31,77.66,133.66a8,8,0,0,1-11.32-11.32L76.69,112,66.34,101.66A8,8,0,0,1,77.66,90.34L88,100.69,98.34,90.34a8,8,0,0,1,11.32,11.32L99.31,112ZM128,192a12,12,0,1,1,12-12A12,12,0,0,1,128,192Zm61.66-69.66a8,8,0,0,1-11.32,11.32L168,123.31l-10.34,10.35a8,8,0,0,1-11.32-11.32L156.69,112l-10.35-10.34a8,8,0,0,1,11.32-11.32L168,100.69l10.34-10.35a8,8,0,0,1,11.32,11.32L179.31,112Z"></path></svg>' . esc_html__('Oops! We couldn\'t find any products matching your search. Try adjusting your filters or explore our full collection.', 'freska') . '
										<a href="#" class="bt-reset-filter-product-btn">' . esc_html__('Reset All Filters', 'freska') . '</a>
									</div>';
        $output['pagination'] = '';
        $output['pagination_meta'] = array(
            'current_page' => 1,
...
add_action('wp_ajax_freska_products_filter', 'freska_products_filter');
add_action('wp_ajax_nopriv_freska_products_filter', 'freska_products_filter');
...
add_action('wp_ajax_freska_products_compare', 'freska_products_compare');
add_action('wp_ajax_nopriv_freska_products_compare', 'freska_products_compare');

function freska_products_wishlist()
{
    ...
    <?php
        ...
        $output['count'] = 0;
        $output['items'] = '<div class="bt-no-results">' . __('No products found! ', 'freska') . '<a href="/shop/">' . __('Back to Shop', 'freska') . '</a></div>';
    }
...
add_action('wp_ajax_freska_products_wishlist', 'freska_products_wishlist');
add_action('wp_ajax_nopriv_freska_products_wishlist', 'freska_products_wishlist');

function freska_products_quick_view()
{
    ...
    <div class="bt-quickview-title">
        <h2><?php echo esc_html($source === 'add-to-cart' ? __('Select options', 'freska') : __('Quick View', 'freska')); ?></h2>
...
add_action('wp_ajax_freska_products_quick_view', 'freska_products_quick_view');
add_action('wp_ajax_nopriv_freska_products_quick_view', 'freska_products_quick_view');

/* get price freeship */
function freska_get_free_shipping_minimum_amount()
{
...
function freska_get_free_shipping()
{
    $free_shipping_threshold = freska_get_free_shipping_minimum_amount();
    ...
    if ($cart_total < $free_shipping_threshold) {
        ...
        $output['message'] = sprintf(
            __('<p class="bt-buy-more">Buy <span>%1$s%2$.2f</span> more to get <span>Free Shipping</span></p>', 'freska'),
            $currency_symbol,
            $amount_left
        );
    } else {
        $output['message'] = __('<p class="bt-congratulation">Congratulations! You have free shipping!</p>', 'freska');
...
add_action('wp_ajax_freska_get_free_shipping', 'freska_get_free_shipping');
add_action('wp_ajax_nopriv_freska_get_free_shipping', 'freska_get_free_shipping');
...
function bt_custom_cross_sells_title($title)
{
    ...
    return !empty($heading) ? esc_html($heading) : esc_html__('You may be interested in…', 'freska');
}
...
/* add button wishlist */
function freska_display_button_wishlist($enable_status, $product_id)
{
    if (!$enable_status) {
        return;
    }
...
/* add button compare */
function freska_display_button_compare($enable_status, $product_id)
{
    if (!$enable_status) {
        return;
    }
...
function freska_display_button_quick_view($enable_status, $product_id)
{
    if (!$enable_status) {
        return;
    }
...
function freska_display_button_wishlist_compare_quick_view()
{
    global $product;
...
add_action('freska_woocommerce_template_loop_list_cta_button', 'freska_display_button_wishlist_compare_quick_view');


/* add button wishlist and compare */
function freska_display_button_wishlist_compare()
{
    global $product;
...
?>
    <div class="bt-product-icon-btn">
        <?php
        echo freska_display_button_compare($show_compare, $product->get_id());
        echo freska_display_button_wishlist($show_wishlist, $product->get_id());
        ?>
    </div>
<?php
}

add_action('woocommerce_after_add_to_cart_button', 'freska_display_button_wishlist_compare');

// WooCommerce custom field
add_action('woocommerce_product_options_advanced', 'freska_woocommerce_custom_field');

function freska_woocommerce_custom_field()
{
    global $post, $product_object;
    // Loop Product Display Options
    echo '<div class="options_group">';
    echo '<h4 style="padding: 10px 12px; margin: 0; border-bottom: 1px solid #eee;">' . __('Loop Product Display', 'freska') . '</h4>';

    woocommerce_wp_checkbox(array(
        'id' => '_enable_loop_sale_marquee',
        'label' => __('Enable Sale Marquee', 'freska'),
        'description' => __('Show sale percentage marquee in product loop', 'freska'),
        'desc_tip' => true
    ));

    woocommerce_wp_checkbox(array(
        'id' => '_enable_loop_countdown',
        'label' => __('Enable Countdown Timer', 'freska'),
        'description' => __('Show countdown timer in product loop (requires countdown date below)', 'freska'),
        'desc_tip' => true
    ));
    ...
    $layout_options = array(
        'bottom-thumbnail' => __('Bottom Thumbnail', 'freska'),
        'left-thumbnail' => __('Left Thumbnail', 'freska'),
        'right-thumbnail' => __('Right Thumbnail', 'freska'),
        'gallery-one-column' => __('Gallery 1 Column', 'freska'),
        'gallery-two-columns' => __('Gallery 2 Columns', 'freska'),
        'gallery-three-columns' => __('Gallery 3 Columns', 'freska'),
        'gallery-four-columns' => __('Gallery 4 Columns', 'freska'),
        'gallery-grid-fullwidth' => __('Gallery Grid Fullwidth', 'freska'),
        'gallery-stacked' => __('Gallery Stacked', 'freska'),
        'gallery-slider-container' => __('Gallery Slider Container', 'freska'),
        'gallery-slider-fullwidth' => __('Gallery Slider Fullwidth', 'freska')
    );
    ...
    woocommerce_wp_select(array(
        'id' => '_layout_product',
        'label' => __('Product Layout', 'freska'),
        'description' => '',
        'options' => $layout_options,
        'value' => $layout_value
    ));

    // layout Show More Settings (only for layouts)
    echo '<div class="options_group">';


    $info_display_options = array(
        'tab' => __('Tab', 'freska'),
        'toggle' => __('Toggle', 'freska'),
        'disable' => __('Disable', 'freska')
    );
    ...
    woocommerce_wp_select(array(
        ...
        'label' => __('Product Info Display', 'freska'),
        'description' => __('Tab option is only available for Thumbnail layouts', 'freska'),
    ...
    woocommerce_wp_select(array(
        ...
        'label' => __('Tab Position', 'freska'),
        'description' => __('Position of tabs (only available for thumbnail layouts)', 'freska'),
        'options' => $tab_position_options,
        'value' => $tab_position_value,
        'desc_tip' => true,
        'wrapper_class' => 'freska_tab_position_field'
    ));

    // Toggle Default State (only for Toggle mode)
    $toggle_state_options = array(
        'first' => __('Open First Toggle', 'freska'),
        'all' => __('Open All Toggles', 'freska')
    );
    ...
    woocommerce_wp_select(array(
        ...
        'label' => __('Toggle Default State', 'freska'),
        'description' => __('Choose which toggles should be open by default', 'freska'),
    ...
    woocommerce_wp_text_input(array(
        'id' => '_gallery_initial_items',
        'label' => __('Initial Items to Show', 'freska'),
        'description' => __('Number of gallery items to show initially before "Show More" button. Leave empty to use layout default.', 'freska'),
        'type' => 'number',
        'custom_attributes' => array(
            'max' => '20',
            'step' => '1'
        ),
        'value' => $initial_items_value,
        'desc_tip' => true
    ));

    $load_more_items_value = get_post_meta($post->ID, '_gallery_load_more_items', true);

    woocommerce_wp_text_input(array(
        'id' => '_gallery_load_more_items',
        'label' => __('Items to Load on Show More', 'freska'),
        'description' => __('Number of additional gallery items to show when "Show More" is clicked. Leave empty to use layout default.', 'freska'),
        'type' => 'number',
        'custom_attributes' => array(
            'max' => '10',
            'step' => '1'
        ),
        'value' => $load_more_items_value,
        'desc_tip' => true
    ));
    ...
    $video_type_options = array(
        'youtube' => __('YouTube', 'freska'),
        'vimeo' => __('Vimeo', 'freska'),
        'mp4' => __('MP4', 'freska')
    );
    ...
    woocommerce_wp_select(array(
        ...
        'label' => __('Product Video Type', 'freska'),
        'description' => '',
    ...
    woocommerce_wp_text_input(array(
        ...
        'label' => __('Product Video Link', 'freska'),
        'description' => __('Enter the video URL (YouTube, Vimeo, or MP4 file URL)', 'freska'),
        'type' => 'url',
        'placeholder' => 'https://'
    ));
    ...
    echo '<label for="_product_360_images">' . __('Product 360° GLB File', 'freska') . '</label>';
    echo '<button type="button" class="button upload_360_images_button">' . __('Upload GLB File', 'freska') . '</button>';
    echo '<input type="hidden" id="_product_360_images" name="_product_360_images" value="' . esc_attr($product_360_file) . '" />';
    echo '<span class="description">' . __('Upload a .glb file for 360° product view', 'freska') . '</span>';
    echo '</p>';
    ...
    if (!empty($product_360_file)) {
        ...
        if ($file_path && file_exists($file_path)) {
            ...
            echo '</div>';
            if ($file_url) {
                echo '<a href="' . esc_url($file_url) . '" target="_blank" class="button button-small" style="margin-left: 10px;">' . __('View', 'freska') . '</a>';
            }
            echo '<button type="button" class="button button-small remove_360_file_button" style="margin-left: 5px; color: #b32d2e;">' . __('Remove', 'freska') . '</button>';
            echo '</div>';
            echo '</div>';
        }
    }
    ...
    woocommerce_wp_checkbox(array(
        ...
        'label' => __('Enable Size Guide', 'freska'),
        'description' => __('Show size guide button for this product', 'freska'),
        'value' => get_post_meta($post->ID, '_enable_size_guide', true)
    ));
    // add product sold
    woocommerce_wp_text_input(array(
        'id' => '_product_sold',
        'label' => __('Product Sold', 'freska'),
        'description' => '',
        'type' => 'number',
        'custom_attributes' => array(
            'min' => 0
    ...
    $label_options = array(
        '' => __('Select Label', 'freska'),
        'best-seller' => __('Best Seller', 'freska'),
        'featured' => __('Featured', 'freska'),
        'new-arrival' => __('New Arrival', 'freska'),
        'on-sale' => __('On Sale', 'freska'),
        'trending' => __('Trending', 'freska'),
        'hot-deal' => __('Hot Deal', 'freska'),
        'pre-order' => __('Pre-Order', 'freska'),
        'exclusive' => __('Exclusive', 'freska'),
        'top-rated' => __('Top Rated', 'freska')
    );
    ...
    if ($product_object && ($product_object->is_type('simple') || $product_object->is_type('variable'))) {
        woocommerce_wp_text_input(array(
            ...
            'label' => __('Date Start Sale', 'freska'),
            'description' => __("Set the date and time when this product's sale price will expire.", 'freska'),
            'type' => 'datetime-local',
            'custom_attributes' => array(
        ...
        woocommerce_wp_text_input(array(
            ...
            'label' => __('Product Date & Time Sale', 'freska'),
            'description' => __("Set the date and time when this product's sale price will expire.", 'freska'),
            'type' => 'datetime-local',
            'custom_attributes' => array(
                'step' => '60',  // 1 minute steps
        ...
        woocommerce_wp_checkbox(array(
            ...
            'label' => __('Disable Sale Price', 'freska'),
            'description' => __('Check this box to automatically remove the sale price when the sale period ends. The product will return to its regular price.', 'freska')
        ));
        woocommerce_wp_checkbox(array(
            'id' => '_enable_percentage_sold',
            'label' => __('Enable Percentage Sold', 'freska'),
            'description' => __('Check this box to display the percentage of products sold.', 'freska')
        ));
        woocommerce_wp_text_input(array(
            'id' => '_product_sold_sale',
            'label' => __('Product Sold Sale', 'freska'),
            'description' => '',
            'type' => 'number',
            'custom_attributes' => array(
                'min' => 0
        ...
        woocommerce_wp_text_input(array(
            ...
            'label' => __('Product Stock Sale', 'freska'),
            'description' => '',
            'type' => 'number',
            'custom_attributes' => array(
                'min' => 0
...
add_action('woocommerce_product_options_related', 'freska_add_frequently_bought_together_field');

function freska_add_frequently_bought_together_field()
{
    global $post;
...
?>
    <p class="form-field">
        <label for="frequently_bought_together_title">
            <?php _e('Title (FBT)', 'freska'); ?>
        </label>
        <input type="text" class="short" name="frequently_bought_together_title" id="frequently_bought_together_title" value="<?php echo esc_attr($fbt_title); ?>" placeholder="<?php esc_attr_e('Title for Frequently Bought Together', 'freska'); ?>" />
        <?php echo wc_help_tip(__('Custom title for the frequently bought together section.', 'freska')); ?>
    </p>
    <p class="form-field">
        <label for="frequently_bought_together_ids">
            <?php _e('Frequently Bought Together', 'freska'); ?>

        </label>
        <select class="wc-product-search" multiple="multiple" style="width: 50%;" id="frequently_bought_together_ids" name="frequently_bought_together_ids[]" data-placeholder="<?php esc_attr_e('Search for a product&hellip;', 'freska'); ?>" data-action="freska_json_search_simple_products" data-exclude="<?php echo intval($post->ID); ?>">
            <?php
        ...
        <?php echo wc_help_tip(__('Select products (simple or variations) that are frequently bought together with this product.', 'freska')); ?>
    </p>
    <?php
}

// AJAX handler to search simple products and variations Frequently Bought Together
add_action('wp_ajax_freska_json_search_simple_products', 'freska_json_search_simple_products');

function freska_json_search_simple_products()
{
...
add_action('woocommerce_process_product_meta', 'freska_woocommerce_custom_field_save');

function freska_woocommerce_custom_field_save($post_id)
{
    ...
    // Save gallery show more settings
    if (isset($_POST['_gallery_initial_items'])) {
        $initial_items = absint($_POST['_gallery_initial_items']);
        if ($initial_items > 0 && $initial_items <= 20) {
            update_post_meta($post_id, '_gallery_initial_items', $initial_items);
        }
    }

    if (isset($_POST['_gallery_load_more_items'])) {
        $load_more_items = absint($_POST['_gallery_load_more_items']);
        if ($load_more_items > 0 && $load_more_items <= 10) {
            update_post_meta($post_id, '_gallery_load_more_items', $load_more_items);
        }
    }
    ...
    $enable_size_guide = isset($_POST['_enable_size_guide']) ? 'yes' : 'no';
...
function freska_convert_title_to_slug($title)
{
...
add_action('freska_woocommerce_shop_loop_item_label', 'freska_woocommerce_product_label', 10);

function freska_woocommerce_product_label()
{
    global $product;
    ...
    if (!empty($label)) {
        echo '<div class="woocommerce-product-label ' . esc_attr(freska_convert_title_to_slug($label)) . '">' . esc_html($label_text) . '</div>';
    }
}
// hook button buy now after add to cart
add_action('woocommerce_after_add_to_cart_button', 'freska_display_button_buy_now');
function freska_display_button_buy_now()
{
    global $product;
    ...
    if ($product->is_type('simple')) {
        if ($product->is_in_stock() && $product->is_purchasable()) {
            ...
            echo '<a href="#" class="button" data-id="' . get_the_ID() . '">' . esc_html__('Buy it now ', 'freska') . '</a>';
            echo '</div>';
        }
        ...
        if ($product->is_type('variable')) {
            ...
            if (!empty($variations)) {
                ...
                $button_class = 'button single_add_to_cart_button bt-btn-buy-now';
                $data_variation = '';
                ...
                echo '<div class="bt-button-buy-now">';
                echo '<a href="#" class="' . esc_attr($button_class) . '" data-id="' . get_the_ID() . '"' . $data_variation . '>' . esc_html__('Buy it now ', 'freska') . '</a>';
                echo '</div>';
            }
        }
        ...
        if ($product->is_type('grouped')) {
            ...
            if (!empty($children)) {
                echo '<div class="bt-button-buy-now">';
                echo '<a href="#" class="button disabled" data-id="' . get_the_ID() . '">' . esc_html__('Buy it now ', 'freska') . '</a>';
                echo '</div>';
            }
        }
    }
...
/* ajax by now product */
add_action('wp_ajax_freska_products_buy_now', 'freska_products_buy_now');
add_action('wp_ajax_nopriv_freska_products_buy_now', 'freska_products_buy_now');

function freska_products_buy_now()
{
...
function freska_products_add_to_cart_variable()
{
    $product_id = intval($_POST['product_id']);
...
add_action('wp_ajax_freska_products_add_to_cart_variable', 'freska_products_add_to_cart_variable');
add_action('wp_ajax_nopriv_freska_products_add_to_cart_variable', 'freska_products_add_to_cart_variable');

/* ajax add to cart simple */

function freska_products_add_to_cart_simple()
{
    $product_id = intval($_POST['product_id']);
    $quantity = intval($_POST['quantity']);
    $fbt_product_id = isset($_POST['fbt_product_id']) ? intval($_POST['fbt_product_id']) : 0;
...
add_action('woocommerce_before_mini_cart', 'freska_calculate_shipping_before_mini_cart', 5);

/* Product share */
if (!function_exists('freska_product_share_render')) {
    function freska_product_share_render()
    {
        $social_item = array();
        $social_item[] = '<li>
            ...
            <div class="bt-product-share">
                <div id="bt_product_share" class="bt-product-share__popup mfp-content__popup mfp-hide">
                    <div class="bt-product-share__content mfp-content__inner">
                        <h3 class="bt-product-share__title"><?php echo esc_html__('Share', 'freska'); ?></h3>
                        <?php
                        ...
                        <div class="bt-product-share__link bt-copy-link-wrap">
                            <h5 class="bt-copy-link-title"><?php echo esc_html__('Copy URL', 'freska'); ?></h5>
                            <form class="bt-product-share-form">
                                <input id="bt-product-share-url" type="text" value="<?php the_permalink(); ?>" readonly="">
                                <button class="button bt-copy-btn" data-copy="<?php echo esc_attr__('Copy', 'freska'); ?>" data-copied="<?php echo esc_attr__('Copied', 'freska'); ?>"><?php echo esc_html__('Copy', 'freska'); ?></button>
                            </form>
                        </div>
                    </div>
                </div>
...
add_action('wp_ajax_freska_remove_section', 'freska_remove_section');
add_action('wp_ajax_nopriv_freska_remove_section', 'freska_remove_section');

function freska_remove_section()
{
    session_start();
...
add_action('wp_ajax_freska_search_live', 'freska_search_live');
add_action('wp_ajax_nopriv_freska_search_live', 'freska_search_live');

// AJAX handler for search product style 1 (grid layout)
function freska_search_live_style1()
{
    $search_term = isset($_POST['search_term']) ? sanitize_text_field($_POST['search_term']) : '';
    $category_slug = isset($_POST['category_slug']) ? sanitize_text_field($_POST['category_slug']) : '';
...
add_action('wp_ajax_freska_search_live_style1', 'freska_search_live_style1');
add_action('wp_ajax_nopriv_freska_search_live_style1', 'freska_search_live_style1');

// Add multiple to cart ajax widget hotspot
function freska_add_multiple_to_cart()
{
    if (!isset($_POST['product_ids']) || empty($_POST['product_ids'])) {
        wp_send_json_error(__('No products selected', 'freska'));
        return;
    }
    ...
    if ($added_count > 0) {
        ...
        wp_send_json_success(array(
            'message' => sprintf(__('%d products added to cart', 'freska'), $added_count),
            'cart_count' => $cart_count
        ));
    } else {
        wp_send_json_error(__('Failed to add products to cart', 'freska'));
    }
}

add_action('wp_ajax_freska_add_multiple_to_cart', 'freska_add_multiple_to_cart');
add_action('wp_ajax_nopriv_freska_add_multiple_to_cart', 'freska_add_multiple_to_cart');
/* add multiple to cart variable widget hotspot normal */
function freska_add_multiple_to_cart_variable()
{
    if (!isset($_POST['product_ids']) || empty($_POST['product_ids'])) {
        wp_send_json_error(__('No products selected', 'freska'));
        return;
    ...
    if (!is_array($product_ids)) {
        wp_send_json_error(__('Invalid product data', 'freska'));
        return;
    }
...
add_action('wp_ajax_freska_add_multiple_to_cart_variable', 'freska_add_multiple_to_cart_variable');
add_action('wp_ajax_nopriv_freska_add_multiple_to_cart_variable', 'freska_add_multiple_to_cart_variable');
// AJAX handler for loading recently viewed products
function freska_load_recently_viewed_products()
{
    ...
    if (!empty($recently_viewed_products)) {
        ...
        echo '<p class="no-products">' . esc_html__('No recently viewed products.', 'freska') . '</p>';
    }
...
add_action('wp_ajax_load_recently_viewed', 'freska_load_recently_viewed_products');
add_action('wp_ajax_nopriv_load_recently_viewed', 'freska_load_recently_viewed_products');

// AJAX handler for loading products display section widget search product style 1
function freska_load_products_display()
{
    ...
    if (!empty($products)) {
        ...
        ob_get_clean();
        $output['content'] = '<p class="no-products">' . esc_html__('No products found.', 'freska') . '</p>';
        $output['has_products'] = false;
    }
...
add_action('wp_ajax_load_products_display', 'freska_load_products_display');
add_action('wp_ajax_nopriv_load_products_display', 'freska_load_products_display');

// Hook the function to run after checkout is completed
add_action('woocommerce_thankyou', 'freska_after_checkout_product', 10, 1);
...
{
    update_post_meta($order_id, '_freska_order_processed', 'no');
}
function freska_after_checkout_product($order_id)
{
    // Check if this order has already been processed
    if (get_post_meta($order_id, '_freska_order_processed', true) === 'yes') {
        return;
    }
    ...
    update_post_meta($order_id, '_freska_order_processed', 'yes');
}
/* hook check sale date countdown product */
function freska_check_sale_date_countdown()
{
    global $product;
...
add_action('woocommerce_before_single_product', 'freska_check_sale_date_countdown', 25);

// Add countdown to single product
add_action('freska_woocommerce_template_single_countdown', 'freska_woocommerce_single_product_countdown', 40);

function freska_woocommerce_single_product_countdown()
{
    global $product;
    $time = get_post_meta($product->get_id(), '_product_datetime', true);
    $stock_status = $product->get_stock_status();
    ...
    if ($time && $stock_status != 'outofstock') {
        ...
        <div class="bt-countdown-product-sale">
            <span class="bt-heading"><?php echo esc_html__('Hurry Up! Offer ends in:', 'freska'); ?></span>
            <div class="bt-countdown bt-countdown-product-js"
                data-idproduct="<?php echo esc_attr($product->get_id()); ?>"
                data-time="<?php echo esc_attr($time); ?>" data-current-time="<?php echo esc_attr($current_time); ?>">
                ...
                <div class="bt-countdown--item">
                    <span class="bt-countdown--digits bt-countdown-days">--</span>
                    <span class="bt-countdown--label"><?php _e('Days', 'freska'); ?></span>
                </div>
                ...
                <div class="bt-countdown--item">
                    <span class="bt-countdown--digits bt-countdown-hours">--</span>
                    <span class="bt-countdown--label"><?php _e('Hours', 'freska'); ?></span>
                </div>
                ...
                <div class="bt-countdown--item">
                    <span class="bt-countdown--digits bt-countdown-mins">--</span>
                    <span class="bt-countdown--label"><?php _e('Mins', 'freska'); ?></span>
                </div>
                ...
                <div class="bt-countdown--item">
                    <span class="bt-countdown--digits bt-countdown-secs">--</span>
                    <span class="bt-countdown--label"><?php _e('Secs', 'freska'); ?></span>
                </div>
            </div>
        </div>
        <?php
        ...
        ?>
            <div class="bt-product-percentage-sold">
                <span class="bt-heading"><?php echo esc_html__('Sold It:', 'freska'); ?></span>
                <div class="bt-product-stock">
                    ...
                    <span class="bt-quantity_sold">
                        <?php printf(esc_html__('%d%% Sold', 'freska'), $percentage); ?> -
                    </span>
                    <span class="bt-stock-text">
                        <?php printf(esc_html__('Only %d item(s) left in stock!', 'freska'), $remaining_products); ?>
                    </span>
                </div>
            </div>
...
add_action('freska_woocommerce_template_single_more_information', 'freska_woocommerce_single_product_more_information', 40);
function freska_woocommerce_single_product_more_information()
{
        ...
        ?>
            <div class="bt-store-location">
                ...
                <a href="#bt_store_location" class="bt-store-location__link bt-js-open-popup-link">
                    ...
                    <span><?php echo esc_html__('View Store Information', 'freska'); ?></span>
                </a>
            </div>
        <?php } ?>
        ...
        <?php if ($delivery_return || $ask_a_question || $product_share) { ?>
            <div class="bt-policy-share">
                ...
                <?php if ($product_share) { ?>
                    <?php echo freska_product_share_render(); ?>
                <?php } ?>
            </div>
        <?php } ?>
    </div>
...
add_action('freska_woocommerce_template_single_safe_checkout', 'freska_woocommerce_single_product_safe_checkout', 40);
function freska_woocommerce_single_product_safe_checkout()
{
    // Check if ACF function exists
    if (!function_exists('get_field')) {
        return;
...
add_action('freska_woocommerce_template_single_out_of_stock', 'freska_woocommerce_single_product_out_of_stock', 10);
function freska_woocommerce_single_product_out_of_stock()
{
    global $product;
    ...
    if (empty($out_of_stock) || empty($out_of_stock['enable_out_of_stock'])) {
        if (!$product->is_type('variable')) {
            echo '<p class="stock out-of-stock">' . esc_html__('Out of stock', 'freska') . '</p>';
        }
        return;
    }
 ...
 */
function freska_handle_thumbnail_layout_mode()
{
    global $product;
    ...
    if ($display_mode === 'tab') {
        // Do nothing here, classes added via freska_add_tab_position_body_class filter
    } elseif ($display_mode === 'toggle') {
        // Toggle mode: Remove default tabs, add toggle
        remove_action('woocommerce_after_single_product_summary', 'woocommerce_output_product_data_tabs', 10);

        // If layout is not thumbnail layout, add to freska_woocommerce_template_single_toggle
        if (!in_array($layout, $thumbnail_layouts)) {
            add_action('freska_woocommerce_template_single_toggle', 'freska_render_product_info_toggle', 10);
        } else {
            add_action('woocommerce_after_single_product_summary', 'freska_render_product_info_toggle', 10);
        }
...
add_action('woocommerce_before_single_product', 'freska_handle_thumbnail_layout_mode', 1);
/**
 * Add tab position to body class for CSS targeting
 * CSS will use body.bt-tabs-position-{position} .woocommerce-tabs selector
 */
function freska_add_tab_position_body_class($classes)
{
    if (is_product()) {
        global $post;
...
add_filter('body_class', 'freska_add_tab_position_body_class');

/**
 * Render product info as TOGGLE/ACCORDION
 * For all layouts that support toggle mode
 */
function freska_render_product_info_toggle()
{
    global $product;
...
add_filter('woocommerce_checkout_fields', 'freska_checkout_field_placeholders');

function freska_checkout_field_placeholders($fields)
{
...
add_action('wp_logout', 'freska_redirect_after_logout');
function freska_redirect_after_logout()
{
...
?>
    <div class="form-row form-row-full variation-gallery-wrapper">
        <h4><?php esc_html_e('Variation Gallery Images', 'freska'); ?></h4>
        ...
        <button type="button" class="button add-variation-gallery-image"><?php esc_html_e('Add Gallery Images', 'freska'); ?></button>
    </div>
    <?php
}

// Save variation gallery data
add_action('woocommerce_save_product_variation', 'freska_save_variation_gallery', 10, 2);

function freska_save_variation_gallery($variation_id, $loop)
{
 ...
 */
function freska_get_product_video_embed($video_type, $video_link)
{
    if (empty($video_link)) {
        return '';
    }
    ...
    switch ($video_type) {
        ...
        case 'mp4':
            ...
            $video_html .= __('Your browser does not support the video tag.', 'freska');
            $video_html .= '</video>';
            $video_html .= '</div>';
            break;
    }
...
function freska_load_product_gallery()
{
    ...
    if ($gallery_layout == 'quickview-slider') {
        ...
        ob_start();
        $html = '<div class="bt-gallery-grid-product__item">' . freska_get_gallery_image_html($main_image_id, true, false) . '</div>';

        if (!empty($gallery_images)) {
            foreach ($gallery_images as $key => $attachment_id) {
                $html .= '<div class="bt-gallery-grid-product__item">' . freska_get_gallery_image_html($attachment_id, true, false) . '</div>';
            }
        }
        echo apply_filters('woocommerce_single_product_image_thumbnail_html', $html, $main_image_id); // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
        ...
        echo '<div class="swiper-wrapper">';
        $html = freska_get_gallery_image_html($main_image_id, false, true);
        foreach ($gallery_images as $key => $attachment_id) {
            $html .= freska_get_gallery_image_html($attachment_id, false, true);
        }
        echo apply_filters('woocommerce_single_product_image_thumbnail_html', $html, $main_image_id); // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
        echo '</div>';
        echo '</div>';
...
add_action('wp_ajax_freska_load_product_gallery', 'freska_load_product_gallery');
add_action('wp_ajax_nopriv_freska_load_product_gallery', 'freska_load_product_gallery');

// ajax load product single variation
function freska_load_product_variation()
{
    if (!isset($_POST['variation_id'])) {
        return;
    }
...
add_action('wp_ajax_freska_load_product_variation', 'freska_load_product_variation');
add_action('wp_ajax_nopriv_freska_load_product_variation', 'freska_load_product_variation');


// ajax load product toast
function freska_load_product_toast()
{
        ...
        <div class="bt-product-toast--content">
            ...
            <div class="bt-product-toast--info">
                <p class="bt-product-toast--title">
                    ...
                    if ($tools === 'wishlist') {
                        $message = $status === 'add'
                            ? esc_html__('has been added to your wishlist.', 'freska')
                            : esc_html__('has been removed from your wishlist.', 'freska');
                    } elseif ($tools === 'compare') {
                        $message = $status === 'add'
                            ? esc_html__('has been added to your compare.', 'freska')
                            : esc_html__('has been removed from your compare.', 'freska');
                    } else {
                        $message = $status === 'add'
                            ? esc_html__('has been added to your cart.', 'freska')
                            : esc_html__('has been removed from your cart.', 'freska');
                    }
...
add_action('wp_ajax_freska_load_product_toast', 'freska_load_product_toast');
add_action('wp_ajax_nopriv_freska_load_product_toast', 'freska_load_product_toast');

/**
 * AJAX: Get current in-cart IDs for sold-individually product (used when cart is updated e.g. item removed).
 */
function freska_get_sold_individually_in_cart()
{
...
add_action('wp_ajax_freska_get_sold_individually_in_cart', 'freska_get_sold_individually_in_cart');
add_action('wp_ajax_nopriv_freska_get_sold_individually_in_cart', 'freska_get_sold_individually_in_cart');

add_action('freska_woocommerce_template_loop_add_to_cart_variable', 'freska_woocommerce_template_loop_add_to_cart_variable', 10);
function freska_woocommerce_template_loop_add_to_cart_variable()
{
    global $product;
    ...
    if ($product->is_type('variable')) {
        ...
        echo esc_html__('Choose Options', 'freska');
        echo '</a>';
    }
}
// hook button add to cart variable after add to cart
add_action('woocommerce_after_add_to_cart_button', 'freska_woocommerce_after_add_to_cart_button', 1);
function freska_woocommerce_after_add_to_cart_button()
{
    global $product;
 ...
 */
add_action('wp_footer', 'freska_single_product_sticky_bar', 20);
function freska_single_product_sticky_bar()
{
                ...
                <div class="bt-single-product-sticky-bar__add-to-cart-wrap bt-single-product-sticky-bar__add-to-cart-wrap--ready <?php echo esc_attr($is_sold_individually ? 'bt-single-product-no-quantity' : ''); ?>" data-mode="add-to-cart" <?php if ($is_variable) {
                    ...
                    <?php if (!$is_sold_individually) { ?>
                        <div class="bt-single-product-sticky-bar__quantity quantity">
                            <span class="qty-minus"><svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512">
                            ...
                            <span class="qty-plus"><svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512">
 ...
 */
function freska_get_frequently_bought_together($product_id = null)
{
    if (empty($product_id)) {
        global $product;
        if (!$product) {
 ...
 * @param int $product_id Product ID
 */
function freska_display_frequently_bought_together($product_id = null)
{
    global $product;
    ...
    $fbt_products = freska_get_frequently_bought_together($product_id);
...
if (!function_exists('freska_register_custom_order_statuses')) {
    function freska_register_custom_order_statuses()
    {
        // Register "Approved" status
        register_post_status('wc-approved', array(
            'label'                     => _x('Approved', 'Order status', 'freska'),
            'public'                    => true,
            'exclude_from_search'       => false,
            'show_in_admin_all_list'    => true,
            'show_in_admin_status_list' => true,
            'label_count'               => _n_noop('Approved <span class="count">(%s)</span>', 'Approved <span class="count">(%s)</span>', 'freska')
        ));

        // Register "Shipped" status
        register_post_status('wc-shipped', array(
            'label'                     => _x('Shipped', 'Order status', 'freska'),
            'public'                    => true,
            'exclude_from_search'       => false,
            'show_in_admin_all_list'    => true,
            'show_in_admin_status_list' => true,
            'label_count'               => _n_noop('Shipped <span class="count">(%s)</span>', 'Shipped <span class="count">(%s)</span>', 'freska')
        ));
    }
    add_action('init', 'freska_register_custom_order_statuses');
}

/* Add Custom Statuses to WooCommerce Order Statuses */
if (!function_exists('freska_add_custom_order_statuses')) {
    function freska_add_custom_order_statuses($order_statuses)
    {
        ...
        foreach ($order_statuses as $key => $status) {
            ...
            if ('wc-on-hold' === $key) {
                $new_order_statuses['wc-approved'] = _x('Approved', 'Order status', 'freska');
            }

            // Add "Shipped" after "Processing"
            if ('wc-processing' === $key) {
                $new_order_statuses['wc-shipped'] = _x('Shipped', 'Order status', 'freska');
            }
        }

        return $new_order_statuses;
    }
    add_filter('wc_order_statuses', 'freska_add_custom_order_statuses');
}

/* Add Custom Statuses to Bulk Actions */
if (!function_exists('freska_add_bulk_actions_custom_statuses')) {
    function freska_add_bulk_actions_custom_statuses($bulk_actions)
    {
        // Add "Approved" status
        $bulk_actions['mark_approved'] = __('Change status to approved', 'freska');

        // Add "Shipped" status  
        $bulk_actions['mark_shipped'] = __('Change status to shipped', 'freska');

        return $bulk_actions;
    }
    add_filter('bulk_actions-edit-shop_order', 'freska_add_bulk_actions_custom_statuses', 20, 1);
}

/* Order Tracking AJAX Handler */
if (!function_exists('freska_track_order_callback')) {
    function freska_track_order_callback()
    {
        // Check nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'freska_order_tracking_nonce')) {
            wp_send_json_error(array(
                'message' => esc_html__('Security check failed.', 'freska')
            ));
        }
        ...
        if (!class_exists('WooCommerce')) {
            wp_send_json_error(array(
                'message' => esc_html__('WooCommerce is not active.', 'freska')
            ));
        }
        ...
        if (empty($order_id) || empty($billing_email)) {
            wp_send_json_error(array(
                'message' => esc_html__('Please fill in all required fields.', 'freska')
            ));
        }
        ...
        if (!$order) {
            wp_send_json_error(array(
                'message' => esc_html__('Order not found. Please check your Order ID.', 'freska')
            ));
        }
        ...
        if (strtolower($order->get_billing_email()) !== strtolower($billing_email)) {
            wp_send_json_error(array(
                'message' => esc_html__('The email address does not match the order. Please try again.', 'freska')
            ));
        }
        ...
        <div class="bt-order-tracking-tabs">
            <div class="bt-tabs-nav">
                <button class="bt-tab-btn active" data-tab="tracking">
                    <?php esc_html_e('TRACK ORDER', 'freska'); ?>
                </button>
                <button class="bt-tab-btn" data-tab="details">
                    <?php esc_html_e('ORDER DETAILS', 'freska'); ?>
                </button>
            </div>
                    ...
                    <div class="bt-timeline-progress">
                        ...
                        <div class="bt-timeline-steps">
                            ...
                            <div class="bt-step <?php echo esc_attr($is_completed); ?> <?php echo esc_attr($is_active); ?>">
                                ...
                                <div class="bt-step-label"><?php esc_html_e('Placed', 'freska'); ?></div>
                            </div>
                            ...
                            <div class="bt-step <?php echo esc_attr($is_completed); ?> <?php echo esc_attr($is_active); ?>">
                                ...
                                <div class="bt-step-label"><?php esc_html_e('Approved', 'freska'); ?></div>
                            </div>
                            ...
                            <div class="bt-step <?php echo esc_attr($is_completed); ?> <?php echo esc_attr($is_active); ?>">
                                ...
                                <div class="bt-step-label"><?php esc_html_e('Processed', 'freska'); ?></div>
                            </div>
                            ...
                            <div class="bt-step <?php echo esc_attr($is_completed); ?> <?php echo esc_attr($is_active); ?>">
                                ...
                                <div class="bt-step-label"><?php esc_html_e('Shipped', 'freska'); ?></div>
                            </div>
                            ...
                            <div class="bt-step <?php echo esc_attr($is_completed); ?> <?php echo esc_attr($is_active); ?>">
                                ...
                                <div class="bt-step-label"><?php esc_html_e('Completed', 'freska'); ?></div>
                            </div>
                        </div>
                    ...
                    <div class="bt-tracking-details">
                        ...
                        ?>
                            <div class="bt-status-section">
                                <h4 class="bt-section-title"><?php esc_html_e('Order Notes', 'freska'); ?></h4>
                                <?php
                ...
                <div class="bt-order-details">
                    <div class="bt-order-info">
                        <div class="bt-order-row">
                            <span class="bt-label"><?php esc_html_e('Order Number:', 'freska'); ?></span>
                            <span class="bt-value">#<?php echo esc_html($order->get_order_number()); ?></span>
                        </div>
                        <div class="bt-order-row">
                            <span class="bt-label"><?php esc_html_e('Order Date:', 'freska'); ?></span>
                            <span class="bt-value"><?php echo esc_html($order->get_date_created()->date_i18n(get_option('date_format'))); ?></span>
                        </div>
                        <div class="bt-order-row">
                            <span class="bt-label"><?php esc_html_e('Total:', 'freska'); ?></span>
                    ...
                    <?php if ($order->get_customer_note()) : ?>
                        <div class="bt-order-note">
                            <h4><?php esc_html_e('Order Note:', 'freska'); ?></h4>
                            <p><?php echo esc_html($order->get_customer_note()); ?></p>
                        </div>
                    <?php endif; ?>

                    <div class="bt-order-items">
                        <h4><?php esc_html_e('Order Items', 'freska'); ?></h4>
                    ...
                    if ($use_shipping) {
                        ...
                        $address_title = __('Shipping Information', 'freska');
                    } else {
                        ...
                        $address_title = __('Billing Information', 'freska');
                    }
                    ?>
                    <div class="bt-order-shipping">
                        ...
                        <?php if (!empty($name)) : ?>
                            <div class="bt-info-row">
                                <span class="bt-info-label"><?php esc_html_e('Name:', 'freska'); ?></span>
                        ...
                        <?php if (!empty($address)) : ?>
                            <div class="bt-info-row">
                                <span class="bt-info-label"><?php esc_html_e('Address:', 'freska'); ?></span>
                        ...
                        <?php if (!empty($phone)) : ?>
                            <div class="bt-info-row">
                                <span class="bt-info-label"><?php esc_html_e('Phone:', 'freska'); ?></span>
                                <span class="bt-info-value">
                        ...
                        <?php if (!empty($email)) : ?>
                            <div class="bt-info-row">
                                <span class="bt-info-label"><?php esc_html_e('Email:', 'freska'); ?></span>
                                <span class="bt-info-value">
...
<?php
        ...
        wp_send_json_success(array(
            'message' => esc_html__('Order found successfully!', 'freska'),
            'html' => $html,
            'current_step' => $current_step
        ));
    }
    add_action('wp_ajax_freska_track_order', 'freska_track_order_callback');
    add_action('wp_ajax_nopriv_freska_track_order', 'freska_track_order_callback');
}

// AJAX Login functionality
if (!function_exists('freska_ajax_login_user')) {
    function freska_ajax_login_user()
    {
        ...
        if (!isset($_POST['bt_login_nonce']) || !wp_verify_nonce($_POST['bt_login_nonce'], 'bt_login_nonce')) {
            wp_send_json_error(['message' => esc_html__('Security check failed.', 'freska')]);
        }
        ...
        if (empty($username) || empty($password)) {
            wp_send_json_error(['message' => esc_html__('Please fill in all required fields.', 'freska')]);
        }
        ...
        wp_send_json_success([
            'message' => esc_html__('Login successful! Redirecting...', 'freska'),
            'redirect_url' => $redirect_url,
        ]);
    }
    add_action('wp_ajax_bt_login_user', 'freska_ajax_login_user');
    add_action('wp_ajax_nopriv_bt_login_user', 'freska_ajax_login_user');
}

// AJAX Register functionality
if (!function_exists('freska_ajax_register_user')) {
    function freska_ajax_register_user()
    {
        // Verify nonce
        ...
        if (!is_email($email)) {
            wp_send_json_error(['message' => esc_html__('Please enter a valid email address.', 'freska')]);
        }

        // Validate username format
        if (!validate_username($username)) {
            wp_send_json_error(['message' => esc_html__('Username contains invalid characters. Only letters, numbers, and underscores are allowed.', 'freska')]);
        }

        // Check username length
        if (strlen($username) < 3) {
            wp_send_json_error(['message' => esc_html__('Username must be at least 3 characters long.', 'freska')]);
        }

        if (strlen($username) > 60) {
            wp_send_json_error(['message' => esc_html__('Username must be less than 60 characters long.', 'freska')]);
        }

        // Check password strength
        if (strlen($password) < 6) {
            wp_send_json_error(['message' => esc_html__('Password must be at least 6 characters long.', 'freska')]);
        }

        if ($password !== $confirm_password) {
            wp_send_json_error(['message' => esc_html__('Passwords do not match. Please make sure both password fields are identical.', 'freska')]);
        }

        if (!$agree_terms) {
            wp_send_json_error(['message' => esc_html__('You must agree to the Terms of Use to create an account.', 'freska')]);
        }

        // Check if username already exists
        if (username_exists($username)) {
            wp_send_json_error(['message' => sprintf(esc_html__('Username "%s" is already taken. Please choose a different username.', 'freska'), $username)]);
        }

        // Check if email already exists
        if (email_exists($email)) {
            wp_send_json_error(['message' => esc_html__('An account with this email address already exists. Please use a different email or try logging in.', 'freska')]);
        }
        ...
        wp_send_json_success([
            'message' => esc_html__('Account created successfully! Redirecting...', 'freska'),
            'redirect_url' => $redirect_url,
        ]);
    }
    add_action('wp_ajax_bt_register_user', 'freska_ajax_register_user');
    add_action('wp_ajax_nopriv_bt_register_user', 'freska_ajax_register_user');
}
