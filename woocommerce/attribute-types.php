 ...
 */
function freska_add_attribute_type_field()
{
	$attribute_type = 'select'; // Default value
	$enable_search = false; // Default value
	?>
	<div class="form-field">
		<label for="freska_attribute_type"><?php esc_html_e('Display Type', 'freska'); ?></label>
		<select name="freska_attribute_type" id="freska_attribute_type">
			<option value="button" <?php selected($attribute_type, 'button'); ?>><?php esc_html_e('Button', 'freska'); ?></option>
			<option value="select" <?php selected($attribute_type, 'select'); ?>><?php esc_html_e('Select', 'freska'); ?></option>
			<option value="image" <?php selected($attribute_type, 'image'); ?>><?php esc_html_e('Image', 'freska'); ?></option>
			<option value="color" <?php selected($attribute_type, 'color'); ?>><?php esc_html_e('Color', 'freska'); ?></option>
		</select>
		<p class="description"><?php esc_html_e('Select how this attribute should be displayed on the frontend.', 'freska'); ?></p>
	</div>
	<div class="form-field">
		<label for="freska_enable_search">
			<input name="freska_enable_search" id="freska_enable_search" type="checkbox" value="1" <?php checked($enable_search, true); ?> />
			<?php esc_html_e('Enable in Search', 'freska'); ?>
		</label>
		<p class="description"><?php esc_html_e('Enable this attribute to be displayed in search/filter fields.', 'freska'); ?></p>
	</div>
	<?php
}
add_action('woocommerce_after_add_attribute_fields', 'freska_add_attribute_type_field');

/**
 * Add attribute type field to edit attribute form
 */
function freska_edit_attribute_type_field()
{
	...
	$attribute_type = get_option('freska_attribute_type_' . $edit, 'select');
	$enable_search = get_option('freska_enable_search_' . $edit, false);
	?>
	...
	<tr class="form-field">
		<th scope="row" valign="top">
			<label for="freska_enable_search"><?php esc_html_e('Enable in Search', 'freska'); ?></label>
		</th>
		<td>
			<label for="freska_enable_search">
				<input name="freska_enable_search" id="freska_enable_search" type="checkbox" value="1" <?php checked($enable_search, true); ?> />
...
add_action('woocommerce_after_edit_attribute_fields', 'freska_edit_attribute_type_field');

/**
 * Save attribute type when adding new attribute
 */
function freska_save_attribute_type($attribute_id, $data = null)
{
	if (isset($_POST['freska_attribute_type'])) {
		$attribute_type = sanitize_text_field($_POST['freska_attribute_type']);
		update_option('freska_attribute_type_' . $attribute_id, $attribute_type);
	...
	$enable_search = isset($_POST['freska_enable_search']) && $_POST['freska_enable_search'] == '1' ? true : false;
	update_option('freska_enable_search_' . $attribute_id, $enable_search);
	
	// Clear static cache for color taxonomies and enabled attributes
	if (function_exists('freska_get_all_color_taxonomies')) {
		// Force refresh by clearing static cache
		$reflection = new ReflectionFunction('freska_get_all_color_taxonomies');
 ...
 */
function freska_update_attribute_type($attribute_id, $data = null, $old_slug = null)
{
	if (isset($_POST['freska_attribute_type'])) {
		$attribute_type = sanitize_text_field($_POST['freska_attribute_type']);
		update_option('freska_attribute_type_' . $attribute_id, $attribute_type);
...
add_action('woocommerce_attribute_updated', 'freska_update_attribute_type', 10, 3);

/**
 * Add metafields to taxonomy terms
 */
function freska_add_term_metafields($taxonomy_name, $attribute_type)
{
	// Register meta for taxonomy terms
	if ($attribute_type === 'image') {
		register_term_meta($taxonomy_name, 'freska_term_image', array(
			'type' => 'integer',
		...
		register_term_meta($taxonomy_name, 'freska_term_color', array(
			'type' => 'string',
 ...
 */
function freska_register_existing_attribute_metafields()
{
 ...
 */
function freska_add_term_metafield_fields($term, $taxonomy = '')
{
	if (empty($taxonomy)) {
	...
	$attribute_type = get_option('freska_attribute_type_' . $attribute_id, 'select');
	
	$term_id = is_object($term) && isset($term->term_id) ? $term->term_id : 0;

	if ($attribute_type === 'image') {
		$image_id = $term_id ? get_term_meta($term_id, 'freska_term_image', true) : 0;
		$image_url = '';
		if ($image_id) {
		...
		<tr class="form-field term-image-wrap">
			<th scope="row">
				<label for="freska_term_image"><?php esc_html_e('Image', 'freska'); ?></label>
			</th>
			<td>
				<div class="freska-term-image-wrapper">
					<div class="freska-term-image-preview" style="margin-bottom: 10px;">
						<?php if ($image_url) : ?>
							<img src="<?php echo esc_url($image_url); ?>" style="max-width: 150px; height: auto; display: block;" />
						<?php endif; ?>
					</div>
					<input type="hidden" name="freska_term_image" id="freska_term_image" value="<?php echo esc_attr($image_id); ?>" />
					<button type="button" class="button freska-upload-image-button"><?php esc_html_e('Select Image', 'freska'); ?></button>
					<button type="button" class="button freska-remove-image-button" style="<?php echo esc_attr($image_id ? '' : 'display:none;'); ?>"><?php esc_html_e('Remove Image', 'freska'); ?></button>
					<p class="description"><?php esc_html_e('Select an image for this term.', 'freska'); ?></p>
				</div>
			</td>
		</tr>
		<?php
	} elseif ($attribute_type === 'color') {
		$color_value = $term_id ? get_term_meta($term_id, 'freska_term_color', true) : '';
		?>
		<tr class="form-field term-color-wrap">
			<th scope="row">
				<label for="freska_term_color"><?php esc_html_e('Color', 'freska'); ?></label>
			</th>
			<td>
				<input type="text" name="freska_term_color" id="freska_term_color" value="<?php echo esc_attr($color_value); ?>" class="freska-color-picker" />
				<p class="description"><?php esc_html_e('Select a color for this term.', 'freska'); ?></p>
			</td>
		</tr>
		<?php
	}
...
function freska_register_term_form_hooks()
{
	if (!function_exists('wc_get_attribute_taxonomies')) {
		return;
	}
	...
	foreach ($attribute_taxonomies as $attribute) {
		...
		add_action($taxonomy_name . '_edit_form_fields', 'freska_add_term_metafield_fields', 10, 2);
		
		// Add form (div format)
		add_action($taxonomy_name . '_add_form_fields', 'freska_add_term_metafield_add_fields', 10, 1);
	}
}
add_action('init', 'freska_register_term_form_hooks', 100);

/**
 * Add image/color fields to term add form (different format from edit)
 */
function freska_add_term_metafield_add_fields($taxonomy)
{
	// Ensure taxonomy is a string
	if (is_array($taxonomy)) {
		return;
	...
	if ($attribute_type === 'image') {
		...
		<div class="form-field term-image-wrap">
			<label for="freska_term_image"><?php esc_html_e('Image', 'freska'); ?></label>
			<div class="freska-term-image-wrapper">
				<div class="freska-term-image-preview" style="margin-bottom: 10px;"></div>
				<input type="hidden" name="freska_term_image" id="freska_term_image" value="" />
				<button type="button" class="button freska-upload-image-button"><?php esc_html_e('Select Image', 'freska'); ?></button>
				<button type="button" class="button freska-remove-image-button" style="display:none;"><?php esc_html_e('Remove Image', 'freska'); ?></button>
				<p class="description"><?php esc_html_e('Select an image for this term.', 'freska'); ?></p>
			</div>
		</div>
		<?php
		...
		<div class="form-field term-color-wrap">
			<label for="freska_term_color"><?php esc_html_e('Color', 'freska'); ?></label>
			<input type="text" name="freska_term_color" id="freska_term_color" value="" class="freska-color-picker" />
			<p class="description"><?php esc_html_e('Select a color for this term.', 'freska'); ?></p>
		</div>
		<?php
	}
 ...
 */
function freska_save_term_metafields($term_id, $tt_id = '', $taxonomy = '')
{
	...
	// Save image
	if ($attribute_type === 'image' && isset($_POST['freska_term_image'])) {
		$image_id = absint($_POST['freska_term_image']);
		update_term_meta($term_id, 'freska_term_image', $image_id);
	}

	// Save color
	if ($attribute_type === 'color' && isset($_POST['freska_term_color'])) {
		$color_value = sanitize_text_field($_POST['freska_term_color']);
		update_term_meta($term_id, 'freska_term_color', $color_value);
	}
}
 ...
 */
function freska_register_term_save_hooks()
{
	if (!function_exists('wc_get_attribute_taxonomies')) {
		return;
	}
	...
	foreach ($attribute_taxonomies as $attribute) {
		$taxonomy_name = wc_attribute_taxonomy_name($attribute->attribute_name);
		add_action('edited_' . $taxonomy_name, 'freska_save_term_metafields', 10, 3);
		add_action('created_' . $taxonomy_name, 'freska_save_term_metafields', 10, 3);
	}
}
add_action('init', 'freska_register_term_save_hooks', 100);

/**
 * Get attribute type for a taxonomy
 */
function freska_get_attribute_type($taxonomy_name)
{
	...
	if ($attribute_id) {
		return get_option('freska_attribute_type_' . $attribute_id, 'select');
	}
 ...
 */
if (!function_exists('freska_get_all_color_taxonomies')) {
	function freska_get_all_color_taxonomies()
	{
		...
		if (!empty($attribute_taxonomies)) {
			foreach ($attribute_taxonomies as $attribute) {
				$attribute_type = get_option('freska_attribute_type_' . $attribute->attribute_id, 'select');
				$enable_search = get_option('freska_enable_search_' . $attribute->attribute_id, false);
 ...
 */
if (!function_exists('freska_get_color_taxonomy')) {
	function freska_get_color_taxonomy()
	{
 ...
 */
if (!function_exists('freska_get_all_enabled_search_attributes')) {
	function freska_get_all_enabled_search_attributes()
	{
...
if (!function_exists('freska_is_attribute_enabled_in_search')) {
	function freska_is_attribute_enabled_in_search($attribute_name_or_id)
	{
		$attribute_id = 0;
	...
	if ($attribute_id) {
		return (bool) get_option('freska_enable_search_' . $attribute_id, false);
	}
 ...
 */
if (!function_exists('freska_sync_acf_filter_choices_on_init')) {
	function freska_sync_acf_filter_choices_on_init()
	{
	...
	function freska_find_and_update_field(&$fields, $target_key, &$found)
	{
		...
		foreach ($fields as &$field) {
			...
			if (isset($field['sub_fields']) && is_array($field['sub_fields'])) {
				freska_find_and_update_field($field['sub_fields'], $target_key, $found);
			}
			
			// Check fields if exists (for groups)
			if (isset($field['fields']) && is_array($field['fields'])) {
				freska_find_and_update_field($field['fields'], $target_key, $found);
			}
		}
	}
}
 ...
 * Hook: freska_bt_attributes_wrap — called from variable.php with $attributes, $product, $available_variations.
 */
if (!function_exists('freska_woocommerce_custom_attributes')) {
	function freska_woocommerce_custom_attributes($attributes, $product, $available_variations)
	{
		...
		foreach ($attributes as $attribute_name => $options) {
			...
			$attr_type = function_exists('freska_get_attribute_type') ? freska_get_attribute_type($attribute_name) : 'select';
				...
				<?php } elseif ($is_color_attr) { ?>
					<div class="bt-attributes--value bt-value-color">
						...
						foreach ($ordered_options as $option) :
							$term = get_term_by('slug', $option, $attribute_name);
							$term_id = $term ? $term->term_id : '';
							$color = $term_id ? get_term_meta($term_id, 'freska_term_color', true) : '';
							if (!$color) {
								$color = $option;
							}
							$is_selected = ($selected_value === $option);
