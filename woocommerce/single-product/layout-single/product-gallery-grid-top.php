...
<div id="product-<?php the_ID(); ?>" <?php wc_product_class('bt-' . $args['layout'], $product); ?>>
    ...
    <div class="<?php echo esc_attr($bt_product_inner_class); ?>" data-parent-image-src="<?php echo esc_url($parent_image_src); ?>">
        ...
        // Get custom gallery settings or use layout defaults
        $custom_initial_items = get_post_meta(get_the_ID(), '_gallery_initial_items', true);
        $custom_load_more_items = get_post_meta(get_the_ID(), '_gallery_load_more_items', true);
        
        if (!empty($custom_initial_items) && $custom_initial_items > 0) {
            $show_number = absint($custom_initial_items);
        } else {
            // Fallback to layout-based defaults
        ...
        // Set load more increment
        $load_more_increment = (!empty($custom_load_more_items) && $custom_load_more_items > 0) ? absint($custom_load_more_items) : 2;

        ?>
        <div class="images bt-gallery-grid-products" data-items="<?php echo esc_attr($itemgallery); ?>" data-shown="<?php echo esc_attr($show_number); ?>" data-load-more="<?php echo esc_attr($load_more_increment); ?>">

            <div class="bt-gallery-grid-product bt-gallery-lightbox bt-gallery-zoomable">
                <?php
                    $html = '<div class="bt-gallery-grid-product__item">' . freska_get_gallery_image_html( $featured_image_id, true, false ) . '</div>';
            ...
            echo '<button class="bt-show-more">' . esc_html__('Show More', 'freska') . '</button>';
            ?>
        </div>
        <div class="summary entry-summary">
            <div class="bt-single-product-info">
                <div class="bt-single-product-info__inner">
                    ...
                    <?php do_action('freska_woocommerce_template_single_title'); ?>
                    ...
                    <div class="bt-product-excerpt-add-to-cart">
                        ...
                        do_action('freska_woocommerce_template_single_countdown'); 
    ...
    <?php
    /**
     * Hook: freska_woocommerce_single_product_after_summary.
     * Fires after the product summary section. Use for extra content, reviews, etc.
     * Does not depend on related products setting.
     *
     * @since 1.0.0
     */
    do_action('freska_woocommerce_single_product_after_summary');

    /**
     * Hook: freska_woocommerce_template_related_products.
     *
    ...
    if (function_exists('get_field')) {
        ...
        if ($enable_related_product) {
            do_action('freska_woocommerce_template_related_products');
        }
    }
    ?>
