...
$is_quick_view = (isset($_REQUEST['action']) && $_REQUEST['action'] === 'freska_products_quick_view');
...
<div class="<?php echo esc_attr(implode(' ', array_map('sanitize_html_class', $wrapper_classes))); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
    ...
    if (is_product()) {
       echo '<div class="bt-button-product-type-wrapper">';
        ...
        if (!empty($video_link) && function_exists('freska_get_product_video_embed')) {
            $video_html = freska_get_product_video_embed($video_type, $video_link);
    ...
    ?>
                <div class="bt-button-product-video">
                    ...
                    <a href="#bt_product_video" class="bt-product-video__link bt-js-open-popup-link" title="<?php echo esc_attr__('Watch Product Video', 'freska'); ?>">
        ...
        if (!empty($product_360_file)) {
            ...
            ?>
                ...
                <div class="bt-button-product-360">
                    ...
                    <!-- 360 Button Trigger -->
                    <a href="#bt_product_360" class="bt-product-360__link bt-js-open-popup-link" title="<?php echo esc_attr__('View 360°', 'freska'); ?>">
    ...
    <div class="woocommerce-product-gallery__wrapper<?php echo (!empty($attachment_ids) && has_post_thumbnail()) ? ' bt-has-slide-thumbs' : ''; ?>">
        ...
        ?>
            <div class="woocommerce-product-gallery__slider bt-gallery-lightbox bt-gallery-zoomable">
                <div class="swiper-wrapper">
                    <?php
                    $html = freska_get_gallery_image_html($post_thumbnail_id, true, true);
            ...
            <div class="woocommerce-product-gallery__slider-thumbs">
                <div class="swiper-wrapper">
                    ...
                    $html = freska_get_gallery_image_html($post_thumbnail_id, false, true);
                    
                    // Add gallery thumbnails
                    if (!empty($attachment_ids)) {
                        foreach ($attachment_ids as $key => $attachment_id) {
                            $html .= freska_get_gallery_image_html($attachment_id, false, true, $key);
                        }
                    }
                    echo apply_filters('woocommerce_single_product_image_thumbnail_html', $html, $post_thumbnail_id); // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
                    ?>
        ...
        <?php
            ...
            $html .= sprintf('<img src="%s" alt="%s" class="wp-post-image" />', esc_url(wc_placeholder_img_src('woocommerce_single')), esc_html__('Awaiting product image', 'freska'));
            $html .= '</div>';
