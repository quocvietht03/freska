...
$related_description   = '';
$related_heading       = __( 'Related Products', 'freska' );
...
if ( $show_section ) :
	...
	$freska_is_related_products = true;
...
?>
		...
		<div class="bt-related-tab-heading">
			<div class="bt-tab-nav">
				...
				<?php if ( $enable_recently_viewed ) : ?>
					<h3 class="bt-main-text bt-tab-title recently-viewed<?php echo empty( $related_products ) ? ' active' : ''; ?>" data-tab="recently-viewed"><?php echo esc_html__( 'Recently Viewed', 'freska' ); ?></h3>
				<?php endif; ?>
			</div>
		</div>
			...
			<div class="bt-tab-pane<?php echo empty( $related_products ) ? ' active' : ''; ?>" data-tab-content="recently-viewed">
				<div class="recently-viewed-products">
					<?php if ( ! empty( $recently_viewed_products ) ) : ?>
						...
						<?php woocommerce_product_loop_end(); ?>
					<?php else : ?>
						<p class="no-products"><?php esc_html_e( 'No recently viewed products.', 'freska' ); ?></p>
					<?php endif; ?>
				</div>
			</div>
			<?php endif; ?>
	...
	<?php
	// Reset global flag
	$freska_is_related_products = false;
endif;
