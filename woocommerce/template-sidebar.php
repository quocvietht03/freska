...
$query_args = freska_products_query_args($query_params, $limit);
...
?>
<?php if (freska_should_show_categories()) { ?>
	<div class="bt-category-wrapper bt-display-<?php echo esc_attr($display_type); ?>">
		<div class="bt-container <?php echo esc_attr($content_width); ?>">
			<div class="bt-category-layout">
				<?php do_action('freska_shop_categories_display'); ?>
			</div>
		</div>
	</div>
<?php } ?>
<?php if (freska_should_show_products()) { ?>
							...
							<div class="bt-products-topbar">
								<div class="bt-product-action">
									<div class="bt-product-filter-toggle">
										...
										</svg><?php esc_html_e('Filters', 'freska') ?>
									</div>
								...
								<div class="bt-product-view-type">
									<?php
									// Determine $type_active from $columns if $_GET['view_type'] is not set
									if (isset($_GET['view_type'])) {
										$type_active = $_GET['view_type'];
									} else {
										// Map from $columns to $type_active
										switch ($columns) {
											case 1:
												$type_active = 'list';
												break;
								...
								<div class="bt-product-orderby">
									<div class="bt-product-sort-block">
										<span class="bt-sort-title">
											<?php echo esc_html__('Sort by:', 'freska'); ?>
										</span>
										<div class="bt-sort-field">
											<?php
											$sort_options = array(
												'date_high' => esc_html__('Date: newest first', 'freska'),
												'date_low' => esc_html__('Date: oldest first', 'freska'),
												'best_selling' => esc_html__('Best Selling', 'freska'),
												'average_rating' => esc_html__('Average Rating', 'freska'),
												'price_high' => esc_html__('Price: highest first', 'freska'),
												'price_low' => esc_html__('Price: lower first', 'freska')
											);
											?>
								...
								<div class="bt-list-tag-filter">
									<a href="#" class="bt-reset-filter-product-btn disable"><?php echo esc_html__('Remove All', 'freska') ?>
								...
								if ($wp_query->have_posts()) {
									...
									if ($pagination_type === 'button-load-more') {
										echo '<div class="bt-product-pagination-wrap" data-pagination-type="load-more">'
											. freska_product_load_more_button($current_page, $total_page)
											. '</div>';
										...
										if ($current_page < $total_page) {
											...
											echo '<span class="bt-loading-spinner" style="display:none;">' . esc_html__('Loading more...', 'freska') . '</span>';
											echo '</div>';
										}
										echo '</div>';
									} else {
										...
										echo '<div class="bt-product-pagination-wrap" data-pagination-type="default">'
											. freska_product_pagination($current_page, $total_page)
											. '</div>';
									}
								} else {
									woocommerce_product_loop_start();
