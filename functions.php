...
function freska_mime_types($mimes)
{
...
add_filter('upload_mimes', 'freska_mime_types');

/* Fix WordPress check filetype for GLB files */
function freska_fix_mime_type_glb($data, $file, $filename, $mimes, $real_mime)
{
...
add_filter('wp_check_filetype_and_ext', 'freska_fix_mime_type_glb', 10, 5);
...
if (!function_exists('freska_enqueue_scripts')) {
	function freska_enqueue_scripts()
	{
		...
		if (class_exists('WooCommerce') && class_exists('WC_AJAX')) {
			...
			wp_localize_script('freska-main', 'wc_cart_params', $wc_cart_params);
		}
...
if (!function_exists('freska_enqueue_admin_scripts')) {
	function freska_enqueue_admin_scripts($hook)
	{
		...
		if ($screen && ($screen->post_type === 'product' || $screen->post_type === 'extra_content_prod')) {
			wp_localize_script('freska-admin-main', 'freskaExtraContent', array(
				'nonce' => wp_create_nonce('freska-extra-content-nonce'),
		...
		if ($screen && $screen->id === 'nav-menus') {
			...
			// Localize script for mega menu admin
			wp_localize_script('freska-admin-main', 'freskaMegamenu', array(
				'ajaxurl' => admin_url('admin-ajax.php'),
				'nonce' => wp_create_nonce('freska-megamenu-nonce'),
				'megamenu' => array(
					'creating' => __('Creating...', 'freska'),
					'newMegaMenu' => __('New Mega Menu', 'freska'),
					'errorCreating' => __('Error creating mega menu', 'freska'),
				),
			));
		}
	}
